...
⏱️ **估計時間**: 40-50 分鐘 | 💰 **成本影響**: ~$5-15/月 | ⭐ **複雜度**: 中階

**📚 學習路徑:**
- ← 上一章: [預檢檢查](preflight-checks.md) - 部署前驗證
- 🎯 **你在這裡**: Application Insights 整合（監控、遙測、除錯）
...
- 使用 **遙測查詢** 來排查生產問題
- 優化 **成本與採樣** 策略
- 監控 **AI/LLM 應用程式**（token、延遲、成本）

## 為什麼在 AZD 中使用 Application Insights 很重要
...
**類比**：Application Insights 就像為你的應用程式安裝了一個「黑盒」飛行記錄器 + 駕駛艙儀表板。你可以即時看到所有發生的事情，並能重播任何事故。
...
| 遙測類型 | 它捕捉的內容 | 使用情境 |
|----------------|------------------|----------|
| **Requests** | HTTP 請求、狀態碼、耗時 | API 效能監控 |
| **Dependencies** | 外部呼叫（資料庫、API、儲存） | 找出瓶頸 |
| **Exceptions** | 未處理錯誤與堆疊追蹤 | 除錯失敗案例 |
| **Custom Events** | 商業事件（註冊、購買） | 分析與漏斗 |
| **Metrics** | 效能計數器、自訂指標 | 容量規劃 |
| **Traces** | 含嚴重性等級的日誌訊息 | 除錯與稽核 |
...
## 課程 1：使用 AZD 自動部署 Application Insights

### AZD 如何建立 Application Insights

當你部署時，AZD 會自動建立並設定 Application Insights。來看看它如何運作。
...
**就是這樣！** AZD 預設會建立 Application Insights。基本監控無需額外設定。
...
if connection_string:
    ...
    middleware = FlaskMiddleware(
        ...
        sampler=ProbabilitySampler(rate=1.0)  # 開發環境使用 100% 取樣
    )
...
對於微服務，Application Insights 會自動把跨服務的請求關聯起來。
...
sequenceDiagram
    ...
    Gateway-->>User: 結帳完成
    Gateway->>AppInsights: 記錄回應 (時長: 450ms)
...
**目標**：追蹤 Azure OpenAI 使用情形（token、成本、延遲）。
...
class MonitoredAzureOpenAI:
    ...
    def chat_completion(self, model: str, messages: list, **kwargs):
        ...
        try:
            ...
            # 計算費用（GPT-4 價格）
            prompt_cost = (prompt_tokens / 1000) * 0.03  # $0.03 每 1K tokens
            completion_cost = (completion_tokens / 1000) * 0.06  # $0.06 每 1K tokens
...
- [ ] **Q1**：AZD 如何部署 Application Insights？
  - **A**：透過 `infra/core/monitoring.bicep` 中的 Bicep 範本自動建立

- [ ] **Q2**：哪個環境變數能啟用 Application Insights？
...
- [ ] **Q2**：事件與指標的差別是什麼？
  - **A**：事件是離散的發生，指標是數值型的量測

- [ ] **Q3**：如何在服務之間關聯遙測？
  - **A**：Application Insights 會自動使用 `operation_Id` 來做關聯
...
- [ ] **Q1**：什麼是採樣，為何要使用？
  - **A**：採樣透過只擷取一部分遙測來減少資料量（及成本）

- [ ] **Q2**：如何設定警示？
  - **A**：使用 Bicep 或 Azure Portal 的指標警示，基於 Application Insights 的指標

- [ ] **Q3**：Log Analytics 與 Application Insights 的差別是什麼？
  - **A**：Application Insights 將資料儲存在 Log Analytics 工作區；App Insights 提供應用程式專屬的檢視
...
**你已學會：**
- ✅ 使用 AZD 自動部署 Application Insights
- ✅ 自訂遙測（事件、指標、依賴）
- ✅ 跨微服務的分散式追蹤
- ✅ 即時指標與實時監控
- ✅ 警示與儀表板
...
**下一步：**
1. 完成實作練習
2. 將 Application Insights 新增到你的 AZD 專案
3. 為你的團隊建立自訂儀表板
...
免責聲明：
本文件已使用 AI 翻譯服務 Co-op Translator（https://github.com/Azure/co-op-translator）進行翻譯。雖然我們力求準確，但請注意自動翻譯可能包含錯誤或不準確之處。原始文件的原文應視為具權威性的版本。對於重要資訊，建議採用專業人工翻譯。我們對因使用本翻譯而引致的任何誤解或錯誤詮釋概不負責。
