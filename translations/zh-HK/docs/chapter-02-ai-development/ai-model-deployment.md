...
- **📖 本章節**：第 2 章 - AI 優先開發
- **⬅️ 上一頁**：[Microsoft Foundry 整合](microsoft-foundry-integration.md)
- **➡️ 下一頁**：[AI 工作坊實作](ai-workshop-lab.md)
- **🚀 下一章**：[第 3 章：配置](../chapter-03-configuration/configuration.md)

本指南提供使用 AZD 範本部署 AI 模型的詳細說明，涵蓋從模型選擇到生產部署模式的所有內容。

> **驗證提示（2026-07-13）：** 本指南中的 AZD 工作流程已針對 `azd` `1.27.1` 版本進行檢查。對於部署時間超過預設服務部署視窗的 AI 部署，當前 AZD 版本支援使用 `azd deploy --timeout <seconds>`。
...
| 模型類型 | 用例 | 建議容量 | 成本考量 |
|------------|----------|---------------------|-------------------|
| gpt-4.1-mini | 聊天、問答 | 10-50 TPM | 在多數工作負載中具成本效益 |
| gpt-4.1 | 複雜推理 | 20-100 TPM | 成本較高，用於高級功能 |
| text-embedding-3-large | 搜索、RAG | 30-120 TPM | 搜索與檢索的強力預設選擇 |
...
1. **查看 [Microsoft Foundry 整合指南](microsoft-foundry-integration.md)** 以學習服務整合模式
2. **完成 [AI 工作坊實作](ai-workshop-lab.md)** 以獲得實務經驗
3. **實施 [生產級 AI 實踐](production-ai-practices.md)** 用於企業部署
