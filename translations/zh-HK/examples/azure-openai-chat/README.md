...
**學習路徑:** 中階 ⭐⭐ | **時間:** 35-45 分鐘 | **費用:** $50-200/月

使用 Azure Developer CLI（azd）部署的完整 Microsoft Foundry Models 聊天應用程式。此範例示範了 gpt-4.1 部署、安全的 API 存取，以及簡單的聊天介面。
...
✅ **Python 聊天應用程式** - 簡易命令列聊天介面  
✅ **Key Vault 整合** - 安全的 API 金鑰儲存  
✅ **ARM 範本** - 完整基礎設施即程式碼  
✅ <strong>費用監控</strong> - 代幣使用量追蹤  
...
```mermaid
graph TD
    App[Python 聊天應用程式<br/>本地/雲端<br/>命令行介面<br/>對話歷史<br/>令牌使用追蹤] -- "HTTPS (API 金鑰)" --> Foundry[Microsoft Foundry 模型服務<br/>gpt-4.1 模型<br/>20K 令牌/分鐘 容量<br/>多區域故障轉移]
...
- **具 OpenAI 訪問權限的 Azure 訂閱** - [申請訪問](https://aka.ms/oai/access)
...
# 驗證 OpenAI 訪問權限（在 Azure 入口網站確認）
az cognitiveservices account list-skus \
  --kind OpenAI \
  --location eastus
```

> **⚠️ 重要:** Microsoft Foundry Models 需要申請審核。如尚未申請，請瀏覽 [aka.ms/oai/access](https://aka.ms/oai/access)。審核通常需 1-2 個工作天。

## ⏱️ 部署時間表

| 階段 | 時間 | 發生事項 |
|-------|----------|--------------|
| 先決條件檢查 | 2-3 分鐘 | 驗證 OpenAI 配額可用性 |
| 部署基礎設施 | 8-12 分鐘 | 建立 OpenAI、Key Vault、模型部署 |
| 設定應用程式 | 2-3 分鐘 | 設定環境和相依性 |
| <strong>總計</strong> | **12-18 分鐘** | 準備好與 gpt-4.1 聊天 |

**注意：** 首次 OpenAI 部署時間較長，因模型配置所致。
...
# 你將會被提示：
# 1. 選擇 Azure 訂閱
# 2. 選擇有 OpenAI 服務的地點（例如，eastus、eastus2、westus）
# 3. 等待 12-18 分鐘完成部署
...
- <strong>代幣計數</strong> - 追蹤使用量並估算費用
- <strong>錯誤處理</strong> - 優雅處理速率限制及 API 錯誤
- <strong>費用估算</strong> - 即時計算每則訊息費用
...
| 使用等級 | 每日訊息數 | 每日代幣數 | 每月費用 |
|-------------|--------------|------------|--------------|
| <strong>輕量</strong> | 20 訊息 | 3,000 代幣 | $3-5 |
| <strong>中量</strong> | 100 訊息 | 15,000 代幣 | $15-25 |
| <strong>大量</strong> | 500 訊息 | 75,000 代幣 | $75-125 |

**基礎基礎設施費用：** $1-2/月（Key Vault + 最小運算）

### 費用優化提示

```bash
# 1. 對較簡單的任務使用 GPT-3.5-Turbo（便宜 20 倍）
export AZURE_OPENAI_MODEL=gpt-35-turbo

# 2. 減少最大 token 數量以縮短回應長度
...
# 應該為: https://[name].openai.azure.com/
```

### 問題：「速率限制超出」

**症狀：** 回傳 429 Too Many Requests
...
```python
# Microsoft Foundry Models 包含內建內容過濾
# 在 Azure 入口網站設定：
...
1. <strong>新增網頁介面</strong> - 建立 React/Vue 前端
   ```bash
   # 將前端服務新增到 azure.yaml
   # 部署至 Azure 靜態 Web 應用程式
   ```

2. **實作 RAG** - 加入 Azure AI Search 文件搜尋
   ```python
   # 整合 Azure AI 搜尋
   # 上傳文件並建立向量索引
   ```
...
- **[資料庫應用程式](../../../../examples/database-app)** - 加入持久化儲存
...
- 📚 [OpenAI API 參考](https://platform.openai.com/docs/api-reference) - API 詳細說明
- 📚 [負責任 AI](https://www.microsoft.com/ai/responsible-ai) - 最佳實務
...
### 工具
- **[Microsoft Foundry Models Studio](https://oai.azure.com/)** - 網頁遊樂場
- **[Prompt 工程指南](https://platform.openai.com/docs/guides/prompt-engineering)** - 編寫更佳提示
- **[代幣計算器](https://platform.openai.com/tokenizer)** - 估算代幣使用

### 社群
- **[Azure AI Discord](https://discord.gg/azure)** - 社群支援
- **[GitHub 討論區](https://github.com/Azure-Samples/openai/discussions)** - 問答論壇
- **[Azure 部落格](https://azure.microsoft.com/blog/tag/azure-openai-service/)** - 最新資訊

---

**🎉 成功！** 你已部署 Microsoft Foundry Models 並建立運作中的聊天應用程式。開始探索 gpt-4.1 的功能，並嘗試不同提示及使用案例。

**有問題嗎？** [開啟議題](https://github.com/microsoft/AZD-for-beginners/issues) 或參閱 [常見問題](../../resources/faq.md)

**費用提醒：** 測試完成後請記得執行 `azd down` 避免持續產生費用（活躍使用約 $50-100/月）。
