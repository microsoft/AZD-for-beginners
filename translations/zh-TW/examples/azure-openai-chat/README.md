...
**學習路徑：** Intermediate ⭐⭐ | **時間：** 35-45 minutes | **費用：** $50-200/month

一個完整的 Microsoft Foundry Models 聊天應用程式，使用 Azure Developer CLI (azd) 部署。本範例示範 gpt-4.1 的部署、受保護的 API 存取，以及一個簡單的聊天介面。
...
✅ **Python 聊天應用** - 簡單的命令列聊天介面  
✅ **Key Vault 整合** - 安全的 API 金鑰儲存  
✅ **ARM 範本** - 完整的基礎架構即程式碼  
✅ <strong>成本監控</strong> - Token 使用追蹤  
...
```mermaid
graph TD
    App[Python 聊天應用程式<br/>本地/雲端<br/>命令列介面<br/>對話記錄<br/>Token 使用追蹤] -- "HTTPS（API 金鑰）" --> Foundry[微軟 Foundry 模型服務<br/>gpt-4.1 模型<br/>每分鐘 20K 令牌容量<br/>多區域故障轉移]
...
- **Azure 訂閱** 且具備 OpenAI 存取權 - [Request access](https://aka.ms/oai/access)
...
|-------|----------|--------------|
| Prerequisites check | 2-3 minutes | 驗證 OpenAI 配額可用性 |
| Deploy infrastructure | 8-12 minutes | 建立 OpenAI、Key Vault、模型部署 |
| Configure application | 2-3 minutes | 設定環境與相依套件 |
| **Total** | **12-18 minutes** | 準備與 gpt-4.1 聊天 |

**注意：** 首次 OpenAI 部署可能因模型配置而需要較長時間。
...
- **Conversation History** - 維持訊息間的上下文
- **Token Counting** - 追蹤使用量並預估成本
- **Error Handling** - 優雅處理速率限制與 API 錯誤
- **Cost Estimation** - 即時計算每則訊息的成本
...
```bash
# 1. 對於較簡單的任務使用 GPT-3.5-Turbo（便宜 20 倍）
export AZURE_OPENAI_MODEL=gpt-35-turbo
...
```bash
# 在 Azure 入口網站：
# OpenAI 資源 → 指標 → 選取「Token Transaction」
...
```python
# Microsoft Foundry 模型包含內建的內容過濾功能
# 在 Azure 入口網站中設定：
...
2. **實作 RAG** - 新增使用 Azure AI Search 的文件搜尋
   ```python
   # 整合 Azure AI Search
   # 上傳文件並建立向量索引
   ```
...
- **[Retail Multi-Agent](../retail-scenario.md)** - 進階的多代理架構
- **[Database App](../../../../examples/database-app)** - 新增永久性儲存
...
- **[Chat Completions](https://learn.microsoft.com/azure/ai-services/openai/how-to/chatgpt)** - 建立聊天應用
- **[Function Calling](https://learn.microsoft.com/azure/ai-services/openai/how-to/function-calling)** - 進階功能

### 工具
- **[Microsoft Foundry Models Studio](https://oai.azure.com/)** - 網頁式 playground
- **[Prompt Engineering Guide](https://platform.openai.com/docs/guides/prompt-engineering)** - 撰寫更好提示的指南
...
**成本提醒：** 測試結束後請記得執行 `azd down` 以避免持續費用（活動使用約 $50-100/month）。
