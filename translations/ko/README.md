...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-19T18:40:55+00:00",
...
Azure Developer CLI (azd)를 체계적인 챕터를 통해 마스터하세요. **Microsoft Foundry 통합을 활용한 AI 애플리케이션 배포에 특별 초점.**

### 이 과정이 현대 개발자에게 필수적인 이유

Microsoft Foundry Discord 커뮤니티의 통찰에 따르면, **개발자의 45%가 AZD를 AI 워크로드에 사용하고 싶어하지만** 다음과 같은 문제를 겪고 있습니다:
- 복잡한 다중 서비스 AI 아키텍처
- 프로덕션 AI 배포 모범 사례  
...
- **코드로 인프라 구현**: Bicep 템플릿을 사용한 Azure 리소스 관리
...
# 검색 통합이 작동하는지 확인합니다
azd monitor  # Application Insights가 원격 분석을 표시하는지 확인합니다
azd down --force --purge
```

**📊 투자 시간:** 1-2시간  
**📈 학습 후 수준:** 프로덕션 준비된 AI 애플리케이션 배포 및 구성 가능  
**💰 비용 인식:** 개발 비용 $80-150/월, 프로덕션 비용 $300-3500/월 예상

#### 💰 AI 배포 비용 고려 사항

**개발 환경 (예상 $80-150/월):**
- Azure OpenAI (사용량 기반): $0-50/월 (토큰 사용량에 따라)
- AI 검색 (기본 등급): $75/월
- 컨테이너 앱 (소비 기반): $0-20/월
- 스토리지 (표준): $1-5/월

**프로덕션 환경 (예상 $300-3,500+/월):**
- Azure OpenAI (PTU로 일관된 성능 제공): $3,000+/월 또는 고용량 사용량 기반
- AI 검색 (표준 등급): $250/월
- 컨테이너 앱 (전용): $50-100/월
- 애플리케이션 인사이트: $5-50/월
- 스토리지 (프리미엄): $10-50/월
...
**💡 비용 최적화 팁:**
- 학습을 위해 **무료 등급** Azure OpenAI 사용 (50,000 토큰/월 포함)
...
**💡 챕터 결과**: 고객 및 재고 에이전트를 포함한 프로덕션 준비 완료된 다중 에이전트 AI 솔루션 배포 및 관리
...
### 인터랙티브 워크숍 자료
**브라우저 기반 도구와 가이드된 실습을 통한 종합적인 실습 학습**

워크숍 자료는 위의 챕터 기반 커리큘럼을 보완하는 구조화된 인터랙티브 학습 경험을 제공합니다. 워크숍은 자율 학습과 강사 주도 세션 모두에 적합하게 설계되었습니다.

#### 🛠️ 워크숍 특징
- **브라우저 기반 인터페이스**: MkDocs 기반 워크숍으로 검색, 복사, 테마 기능 제공
- **GitHub Codespaces 통합**: 원클릭 개발 환경 설정
- **구조화된 학습 경로**: 7단계 가이드 실습 (총 3.5시간)
- **발견 → 배포 → 커스터마이징**: 점진적 방법론
- **인터랙티브 DevContainer 환경**: 사전 구성된 도구 및 종속성

#### 📚 워크숍 구조
워크숍은 **발견 → 배포 → 커스터마이징** 방법론을 따릅니다:
...
- **프로덕션 AI 애플리케이션 배포**: Microsoft Foundry 서비스를 사용하여 AZD 활용
- **다중 에이전트 아키텍처 숙달**: 협력 AI 에이전트 솔루션 구현
...
Azure Developer CLI (azd)는 Azure에 애플리케이션을 빌드하고 배포하는 과정을 가속화하는 개발자 중심의 명령줄 인터페이스입니다. 다음을 제공합니다:
...
- **엔드 투 엔드 AI 워크플로우** - 모델 개발부터 적절한 모니터링을 갖춘 프로덕션 배포까지
...
> **참고:** 이 템플릿은 다양한 AI 패턴을 보여줍니다. 일부는 외부 Azure 샘플이고, 다른 일부는 로컬 구현입니다.

| 템플릿 | 챕터 | 난이도 | 서비스 | 유형 |
|--------|------|--------|--------|------|
| [**AI 채팅 시작하기**](https://github.com/Azure-Samples/get-started-with-ai-chat) | 챕터 2 | ⭐⭐ | AzureOpenAI + Azure AI 모델 추론 API + Azure AI 검색 + Azure Container Apps + Application Insights | 외부 |
| [**AI 에이전트 시작하기**](https://github.com/Azure-Samples/get-started-with-ai-agents) | 챕터 2 | ⭐⭐ | Azure AI 에이전트 서비스 + AzureOpenAI + Azure AI 검색 + Azure Container Apps + Application Insights| 외부 |
| [**Azure 검색 + OpenAI 데모**](https://github.com/Azure-Samples/azure-search-openai-demo) | 챕터 2 | ⭐⭐ | AzureOpenAI + Azure AI 검색 + App Service + Storage | 외부 |
...
| [**Contoso Chat RAG**](https://github.com/Azure-Samples/contoso-chat) | 챕터 8 | ⭐⭐⭐⭐ | AzureOpenAI + AI 검색 + Cosmos DB + Container Apps | 외부 |
| [**소매 다중 에이전트 솔루션**](examples/retail-scenario.md) | 챕터 5 | ⭐⭐⭐⭐ | AzureOpenAI + AI 검색 + Storage + Container Apps + Cosmos DB | **로컬** |

### 추천: 완전한 학습 시나리오
**학습 챕터에 매핑된 프로덕션 준비 완료된 애플리케이션 템플릿**
...
| [**retail-multi-agent-solution**](examples/retail-scenario.md) | 챕터 5 | ⭐⭐⭐⭐ | 고객 및 재고 에이전트를 포함한 다중 에이전트 아키텍처 |
...
#### 로컬 예제 (즉시 사용 가능)
- [**소매 다중 에이전트 솔루션**](examples/retail-scenario.md) - ARM 템플릿을 사용한 완전한 프로덕션 준비 구현
...
- [**컨테이너 앱 예제**](examples/container-app/README.md) - 컨테이너화된 배포에 대한 완전한 가이드
  ...
  - [마이크로서비스 아키텍처](../../examples/container-app/microservices) - 프로덕션 준비 완료된 다중 서비스 배포
  - 빠른 시작, 프로덕션, 고급 배포 패턴
...
- [엔터프라이즈 ML 파이프라인](https://github.com/Azure-Samples/mlops-v2) - 프로덕션 준비 완료된 ML 패턴
...
- [**예제 디렉토리**](examples/README.md) - 상세한 설명이 포함된 로컬 학습 예제
...
- [**FAQ**](resources/faq.md) - 학습 챕터별 일반적인 질문
- [**학습 가이드**](resources/study-guide.md) - 종합적인 실습 문제
### 실습 워크숍
- [**AI 워크숍 랩**](docs/microsoft-foundry/ai-workshop-lab.md) - AI 솔루션을 AZD로 배포 가능하게 만들기 (2-3시간)
...
<details>
<summary><strong>❌ "azd up" 실행 중간에 실패</strong></summary>
...
[![IoT 초보자를 위한 가이드](https://img.shields.io/badge/IoT%20for%20Beginners-14B8A6?style=for-the-badge&labelColor=E5E7EB&color=14B8A6)](https://aka.ms/iot-beginners?WT.mc_id=academic-105485-koreyst)  
[![XR 초보자를 위한 개발](https://img.shields.io/badge/XR%20Development%20for%20Beginners-38BDF8?style=for-the-badge&labelColor=E5E7EB&color=38BDF8)](https://github.com/microsoft/xr-development-for-beginners?WT.mc_id=academic-105485-koreyst)
