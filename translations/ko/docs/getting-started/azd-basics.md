...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-17T14:33:28+00:00",
...
이 강의에서는 Azure Developer CLI(azd)를 소개합니다. azd는 로컬 개발에서 Azure 배포까지의 여정을 가속화하는 강력한 명령줄 도구입니다. 기본 개념과 핵심 기능을 배우고, azd가 클라우드 네이티브 애플리케이션 배포를 어떻게 간소화하는지 이해하게 됩니다.

## 학습 목표

이 강의를 마치면 다음을 할 수 있습니다:
- Azure Developer CLI가 무엇인지와 주요 목적을 이해하기
- 템플릿, 환경, 서비스의 핵심 개념 배우기
- 템플릿 기반 개발 및 코드로서의 인프라(Infrastructure as Code)와 같은 주요 기능 탐구하기
- azd 프로젝트 구조와 워크플로를 이해하기
- 개발 환경을 위해 azd를 설치하고 구성할 준비하기

## 학습 결과

이 강의를 완료한 후, 다음을 할 수 있습니다:
- 현대 클라우드 개발 워크플로에서 azd의 역할 설명하기
- azd 프로젝트 구조의 구성 요소 식별하기
- 템플릿, 환경, 서비스가 어떻게 함께 작동하는지 설명하기
- azd를 사용한 코드로서의 인프라의 이점 이해하기
- 다양한 azd 명령과 그 목적을 인식하기
...
`azd down --force --purge` 명령은 azd 환경과 관련된 모든 리소스를 완전히 제거하는 강력한 방법입니다. 각 플래그가 수행하는 작업은 다음과 같습니다:
```
--force
```
- 확인 프롬프트를 건너뜁니다.
...
### 왜 둘 다 사용해야 할까?
`azd up`에서 남아 있는 상태나 부분 배포로 인해 문제가 발생했을 때, 이 조합은 **완전한 초기화**를 보장합니다.

특히 Azure 포털에서 수동으로 리소스를 삭제하거나 템플릿, 환경, 리소스 그룹 명명 규칙을 변경할 때 유용합니다.
...
- 기존 템플릿으로 시작하기
- 필요에 맞게 사용자 정의하기
- 조직을 위한 재사용 가능한 템플릿 생성하기

### 3. 환경 분리
- 개발/스테이징/프로덕션 환경을 분리하여 사용하기
- 로컬 머신에서 직접 프로덕션에 배포하지 않기
- 프로덕션 배포에는 CI/CD 파이프라인 사용하기

### 4. 구성 관리
- 민감한 데이터에는 환경 변수를 사용하기
- 구성 파일을 버전 관리에 포함하기
- 환경별 설정을 문서화하기
...
**🎯 다음 챕터 준비하기:**
- [챕터 2: AI 우선 개발](../ai-foundry/azure-ai-foundry-integration.md) - AI 애플리케이션 빌드 시작하기
