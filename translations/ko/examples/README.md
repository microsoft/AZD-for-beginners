...
- **🚀 로컬 예제**: [리테일 다중 에이전트 솔루션](retail-scenario.md)
- **🤖 외부 AI 예제**: Azure 샘플 저장소 링크

> **📍 중요: 로컬 예제 vs 외부 예제**  
> 이 저장소에는 <strong>4개의 완전한 로컬 예제</strong>가 전체 구현과 함께 포함되어 있습니다:  
> - **Microsoft Foundry Models 채팅** (gpt-4.1 배포와 채팅 인터페이스)  
> - **컨테이너 앱** (간단한 Flask API + 마이크로서비스)  
> - **데이터베이스 앱** (웹 + SQL 데이터베이스)  
> - **리테일 다중 에이전트** (기업용 AI 솔루션)  
>  
> 추가 예제는 클론하여 사용할 수 있는 Azure-Samples 저장소에 대한 <strong>외부 참조</strong>입니다.

## 소개

이 디렉토리는 실습을 통해 Azure Developer CLI를 학습할 수 있도록 실용적인 예제와 참조를 제공합니다. 리테일 다중 에이전트 시나리오는 이 저장소에 포함된 완전한 프로덕션 준비 구현입니다. 추가 예제는 다양한 AZD 패턴을 보여주는 공식 Azure 샘플을 참조합니다.

### 난이도 평가 기준

- ⭐ <strong>초보자</strong> - 기본 개념, 단일 서비스, 15-30분
- ⭐⭐ <strong>중급</strong> - 여러 서비스, 데이터베이스 통합, 30-60분
...
**이 저장소에 완전한 gpt-4.1 배포와 채팅 인터페이스 포함**

- **위치:** `examples/azure-openai-chat/`  
- **난이도:** ⭐⭐ (중급)  
- **포함 내용:**  
  - Microsoft Foundry Models 전체 배포 (gpt-4.1)  
  - Python 명령줄 채팅 인터페이스  
  - 안전한 API 키를 위한 Key Vault 통합  
  - Bicep 인프라 템플릿  
  - 토큰 사용량 및 비용 추적  
  - 속도 제한 및 오류 처리  
...
#### [리테일 다중 에이전트 솔루션](retail-scenario.md) 🆕  
...
다음 예제들은 공식 Azure-Samples 저장소에서 관리됩니다. 클론하여 다양한 AZD 패턴을 탐색하세요:
...
| <strong>마이크로서비스</strong> | [로컬: microservices](../../../examples/container-app/microservices) | ⭐⭐⭐⭐ | 다중 서비스, Service Bus, Cosmos DB, SQL |
...
이 예제를 통해:
- 실제 애플리케이션 시나리오로 Azure Developer CLI 워크플로우 실습  
- 다양한 애플리케이션 아키텍처 및 azd 구현 이해  
- 다양한 Azure 서비스의 Infrastructure as Code 패턴 숙달  
- 구성 관리 및 환경별 배포 전략 적용  
- 실제 상황에서 모니터링, 보안, 확장 패턴 구현  
- 실제 배포 문제 해결 및 디버깅 경험 구축

## 학습 성과

예제 완료 후:
- Azure Developer CLI를 사용한 다양한 애플리케이션 유형 자신감 있게 배포  
- 제공된 템플릿을 자신의 애플리케이션 요구사항에 맞게 조정  
- Bicep을 사용한 맞춤형 인프라 패턴 설계 및 구현  
- 적절한 종속성으로 복잡한 다중 서비스 애플리케이션 구성  
- 실제 시나리오에서 보안, 모니터링 및 성능 모범 사례 적용  
- 실무 경험을 바탕으로 배포 문제 해결 및 최적화 가능
...
> **💡 AZD가 처음인가요?** 예제 #1 (Flask API)부터 시작하세요 - 약 20분 소요, 핵심 개념을 배울 수 있습니다.
...
1. **[컨테이너 앱 - Python Flask API](../../../examples/container-app/simple-flask-api)** (로컬) ⭐  
   ...
   **예상 결과:** "Hello, World!"를 반환하는 작동하는 API 엔드포인트 및 모니터링

2. **[간단한 웹 앱 - Node.js Express](https://github.com/Azure-Samples/todo-nodejs-mongo)** ⭐  
   MongoDB와 함께하는 Node.js Express 웹 애플리케이션 배포  
   **시간:** 25-35분 | **비용:** $10-30/월  
   **학습 내용:** 데이터베이스 통합, 환경 변수, 연결 문자열  
   **예상 결과:** CRUD 기능이 있는 Todo 리스트 앱
...
1. **[Microsoft Foundry Models 채팅 앱 - 로컬 예제](../../../examples/azure-openai-chat)** ⭐⭐  
   채팅 인터페이스가 포함된 완전한 gpt-4.1 배포  
   **시간:** 35-45분 | **비용:** $50-200/월  
   **예상 결과:** 토큰 추적과 비용 모니터링이 가능한 작동 채팅 애플리케이션

2. **[Azure Search + OpenAI 데모](https://github.com/Azure-Samples/azure-search-openai-demo)** ⭐⭐⭐  
   RAG 아키텍처 기반 지능형 채팅 애플리케이션  
   **시간:** 60-90분 | **비용:** $100-300/월  
   **예상 결과:** 문서 검색 및 인용 기능이 있는 RAG 기반 채팅 인터페이스

3. **[AI 문서 처리](https://github.com/Azure-Samples/azure-ai-document-processing)** ⭐⭐  
   Azure AI 서비스를 이용한 문서 분석  
   **시간:** 40-60분 | **비용:** $20-80/월  
   **예상 결과:** 업로드된 문서에서 텍스트, 표, 엔티티를 추출하는 API

4. **[머신러닝 파이프라인](https://github.com/Azure-Samples/mlops-v2)** ⭐⭐⭐⭐  
   Azure Machine Learning을 이용한 MLOps 워크플로우  
   **시간:** 2-3시간 | **비용:** $150-500/월  
   **예상 결과:** 학습, 배포 및 모니터링이 자동화된 ML 파이프라인
...
AZD로 엔터프라이즈 급 AI 애플리케이션 배포를 보여주는 포괄적인 프로덕션 준비 다중 에이전트 고객 지원 솔루션입니다. 이 시나리오는 다음을 제공합니다:

- **완전한 아키텍처**: 전문화된 고객 서비스 및 재고 관리 에이전트를 갖춘 다중 에이전트 시스템
- **운영 인프라**: 다중 지역 Microsoft Foundry Models 배포, AI 검색, 컨테이너 앱, 종합 모니터링  
- **즉시 배포 가능한 ARM 템플릿**: 다중 구성 모드(최소/표준/프리미엄)를 지원하는 원클릭 배포  
- **고급 기능**: Red teaming 보안 검증, 에이전트 평가 프레임워크, 비용 최적화, 문제 해결 가이드  
- **실제 비즈니스 컨텍스트**: 파일 업로드, 검색 통합, 동적 확장 기능을 갖춘 소매업 고객 지원 사용 사례  
...
<strong>복잡도</strong>: ⭐⭐⭐⭐ (고급 - 기업 운영 준비 완료)  

**추천 대상**: AI 개발자, 솔루션 아키텍트, 운영용 다중 에이전트 시스템 구축 팀  

**빠른 시작**: 포함된 ARM 템플릿을 사용하여 `./deploy.sh -g myResourceGroup` 명령어로 30분 이내에 전체 솔루션 배포  
...
모든 예제를 실행하기 전에:  
- ✅ Owner 또는 Contributor 권한이 있는 Azure 구독  
- ✅ Azure Developer CLI 설치 ([설치 가이드](../docs/chapter-01-foundation/installation.md))  
- ✅ Docker Desktop 실행 중(컨테이너 예제용)  
- ✅ 적합한 Azure 할당량(예제별 요구사항 확인)  

> **💰 비용 경고:** 모든 예제는 실제 Azure 리소스를 생성하며 요금이 발생합니다. 자세한 비용 추정치는 각 README 파일을 참고하세요. 완료 후에는 `azd down`을 실행하여 지속 비용 발생을 방지하세요.  
...
5. **배포 검증**  
   ...
   **성공 기대 사항:**  
   ...
   - ✅ Application Insights가 텔레메트리 수신  

> **⚠️ 문제 발생 시?** 배포 문제 해결 방법은 [일반 문제](../docs/chapter-07-troubleshooting/common-issues.md)를 참고하세요.  

### 예제 맞춤화  

각 예제는 다음을 포함합니다:  
- **README.md** - 상세 설치 및 사용자 정의 안내  
- **azure.yaml** - 주석 포함 AZD 구성  
- **infra/** - 파라미터 설명이 포함된 Bicep 템플릿  
- **src/** - 샘플 애플리케이션 코드  
- **scripts/** - 일반 작업용 보조 스크립트  
...
<strong>컨셉</strong>: 기본 배포, REST API, NoSQL 데이터베이스 통합  

### 정적 웹사이트 - React SPA  
**기술 스택**: React, Azure Static Web Apps, Azure Functions, Cosmos DB  
<strong>복잡도</strong>: 초급  
<strong>컨셉</strong>: 정적 호스팅, 서버리스 백엔드, 최신 웹 개발  

### 컨테이너 앱 - Python Flask  
**기술 스택**: Python Flask, Docker, Container Apps, Container Registry, Application Insights  
<strong>복잡도</strong>: 초급  
<strong>컨셉</strong>: 컨테이너화, REST API, scale-to-zero, 헬스 프로브, 모니터링  
...
| AI 문서 처리 | 2 | ❌ | 기본 | 전체 | ⭐⭐ |  
| ML 파이프라인 | 4+ | ✅ | 전체 | 전체 | ⭐⭐⭐⭐ |  
| **소매 다중 에이전트** (로컬) | **8+** | **✅** | <strong>엔터프라이즈</strong> | <strong>고급</strong> | **⭐⭐⭐⭐** |  
...
- **Python**: [Python Flask API (로컬)](../../../examples/container-app/simple-flask-api), [마이크로서비스 제품 서비스 (로컬)](../../../examples/container-app/microservices), Python Functions + SPA  
- **C#**: [마이크로서비스 주문 서비스 (로컬)](../../../examples/container-app/microservices), C# Web API + SQL 데이터베이스, Microsoft Foundry Models 채팅 앱, ML 파이프라인  
...
- **AI/ML**: **[Microsoft Foundry Models 채팅 (로컬)](../../../examples/azure-openai-chat)**, Microsoft Foundry Models 채팅 앱, AI 문서 처리, ML 파이프라인, **소매 다중 에이전트 솔루션**  
- **다중 에이전트 시스템**: **소매 다중 에이전트 솔루션**  
- **OpenAI 통합**: **[Microsoft Foundry Models 채팅 (로컬)](../../../examples/azure-openai-chat)**, 소매 다중 에이전트 솔루션  
- **엔터프라이즈 운영 환경**: [마이크로서비스 (로컬)](../../../examples/container-app/microservices), **소매 다중 에이전트 솔루션**  

### 아키텍처 패턴별  
- **간단한 REST API**: [Python Flask API (로컬)](../../../examples/container-app/simple-flask-api)  
- **단일 모놀리식**: Node.js Express Todo, C# Web API + SQL  
- **정적 + 서버리스**: React SPA + Functions, Python Functions + SPA  
- <strong>마이크로서비스</strong>: [운영용 마이크로서비스 (로컬)](../../../examples/container-app/microservices), Java Spring Boot 마이크로서비스  
- <strong>컨테이너화된</strong>: [Python Flask (로컬)](../../../examples/container-app/simple-flask-api), [마이크로서비스 (로컬)](../../../examples/container-app/microservices)  
- **AI 기반**: **[Microsoft Foundry Models 채팅 (로컬)](../../../examples/azure-openai-chat)**, Microsoft Foundry Models 채팅 앱, AI 문서 처리, ML 파이프라인, **소매 다중 에이전트 솔루션**  
...
- <strong>중급</strong>: **[Microsoft Foundry Models 채팅 (로컬)](../../../examples/azure-openai-chat)**, C# Web API + SQL, Python Functions + SPA, Java 마이크로서비스, Microsoft Foundry Models 채팅 앱, AI 문서 처리  
- <strong>고급</strong>: ML 파이프라인  
- **엔터프라이즈 운영 준비 완료**: [마이크로서비스 (로컬)](../../../examples/container-app/microservices) (메시지 큐가 포함된 다중 서비스), **소매 다중 에이전트 솔루션** (ARM 템플릿 배포가 포함된 완전 다중 에이전트 시스템)  
...
- [Microsoft Foundry 템플릿](https://github.com/Azure/ai-foundry-templates)  
- [Azure Developer CLI 갤러리](https://azure.github.io/awesome-azd/)  
- [C# 및 Azure SQL을 이용한 Todo 앱](https://github.com/Azure-Samples/todo-csharp-sql)  
- [Python 및 MongoDB를 이용한 Todo 앱](https://github.com/Azure-Samples/todo-python-mongo)  
- [Node.js 및 PostgreSQL을 이용한 Todo 앱](https://github.com/Azure-Samples/todo-nodejs-mongo)
- [C# API가 포함된 React 웹 앱](https://github.com/Azure-Samples/todo-csharp-cosmos-sql)
- [Azure Container Apps 작업](https://github.com/Azure-Samples/container-apps-jobs)
- [Java로 작성된 Azure Functions](https://github.com/Azure-Samples/azure-functions-java-flex-consumption-azd)
...
**전문가 팁**: 기술 스택과 일치하는 가장 간단한 예제로 시작하고 점차 더 복잡한 시나리오로 확장하세요. 각 예제는 이전 개념을 기반으로 합니다!

## 🚀 시작할 준비가 되셨나요?

### 학습 경로

1. **완전 초보인가요?** → [Flask API](../../../examples/container-app/simple-flask-api) (⭐, 20분)부터 시작하세요
2. **기본 AZD 지식이 있나요?** → [마이크로서비스](../../../examples/container-app/microservices) (⭐⭐⭐⭐, 60분)를 시도하세요
3. **AI 앱을 구축하고 있나요?** → [Microsoft Foundry Models Chat](../../../examples/azure-openai-chat) (⭐⭐, 35분)부터 시작하거나 [Retail Multi-Agent](retail-scenario.md) (⭐⭐⭐⭐, 2시간 이상)를 탐색하세요
4. **특정 기술 스택이 필요하나요?** → 위의 [적합한 예제 찾기](#-finding-the-right-example) 섹션을 활용하세요

### 다음 단계

- ✅ 위의 [사전 조건](#사전-준비) 검토
- ✅ 자신의 숙련도에 맞는 예제 선택 ([복잡도 평가 도표](#난이도-평가-기준) 참고)
- ✅ 예제 README를 배포 전에 꼼꼼히 읽기
- ✅ 테스트 후 `azd down` 명령 실행 알림 설정
- ✅ GitHub Issues나 Discussions를 통해 경험 공유

### 도움이 필요하신가요?

- 📖 [FAQ](../resources/faq.md) - 자주 묻는 질문 답변
- 🐛 [문제 해결 가이드](../docs/chapter-07-troubleshooting/common-issues.md) - 배포 문제 수정
- 💬 [GitHub Discussions](https://github.com/microsoft/AZD-for-beginners/discussions) - 커뮤니티에 질문
...
- **📖 학습 자료**: [학습 가이드](../resources/study-guide.md) | [요약 노트](../resources/cheat-sheet.md) | [용어집](../resources/glossary.md)
- **🔧 리소스**: [FAQ](../resources/faq.md) | [문제 해결](../docs/chapter-07-troubleshooting/common-issues.md)

---

*최종 업데이트: 2025년 11월 | [이슈 보고하기](https://github.com/microsoft/AZD-for-beginners/issues) | [예제 기여하기](https://github.com/microsoft/AZD-for-beginners/blob/main/CONTRIBUTING.md)*
