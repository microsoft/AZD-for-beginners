...
- gpt-4.1 모델로 Microsoft Foundry Models 서비스 배포  
- Key Vault로 OpenAI API 키 보호  
...
```mermaid
graph TD
    App[파이썬 채팅 애플리케이션<br/>로컬/클라우드<br/>커맨드라인 인터페이스<br/>대화 기록<br/>토큰 사용 추적] -- "HTTPS (API 키)" --> Foundry[마이크로소프트 파운드리 모델 서비스<br/>gpt-4.1 모델<br/>분당 20K 토큰 처리 용량<br/>다중 지역 장애 조치]
...
> **⚠️ 중요:** Microsoft Foundry Models는 애플리케이션 승인이 필요합니다. 신청하지 않은 경우 [aka.ms/oai/access](https://aka.ms/oai/access)에서 방문하세요. 승인에는 일반적으로 1-2 영업일이 소요됩니다.

## ⏱️ 배포 일정

| 단계 | 소요 시간 | 진행 내용 |
|-------|----------|--------------|
| 사전 요구 사항 확인 | 2-3분 | OpenAI 할당량 가능 여부 확인 |
| 인프라 배포 | 8-12분 | OpenAI, Key Vault, 모델 배포 생성 |
| 애플리케이션 구성 | 2-3분 | 환경 및 종속성 설정 |
| <strong>총계</strong> | **12-18분** | gpt-4.1과 채팅 준비 완료 |

**참고:** 최초 OpenAI 배포 시 모델 프로비저닝으로 인해 시간이 더 소요될 수 있습니다.
...
# 다음을 요청받게 됩니다:
# 1. Azure 구독 선택
# 2. OpenAI 사용 가능한 위치 선택(예: eastus, eastus2, westus)
# 3. 배포를 위해 12-18분 대기
...
- **대화 기록 유지** - 메시지 간 컨텍스트 관리  
- **토큰 계산** - 사용량 추적 및 비용 예측  
- **오류 처리** - 속도 제한 및 API 오류 우아한 처리  
...
| 사용 수준 | 하루 메시지 수 | 하루 토큰 수 | 월 비용 |
|-------------|--------------|------------|--------------|
| <strong>가벼움</strong> | 20 메시지 | 3,000 토큰 | $3-5 |
| <strong>보통</strong> | 100 메시지 | 15,000 토큰 | $15-25 |
| <strong>무거움</strong> | 500 메시지 | 75,000 토큰 | $75-125 |

**기본 인프라 비용:** $1-2/월 (Key Vault + 최소 컴퓨트)

### 비용 최적화 팁

```bash
# 1. 더 간단한 작업에는 GPT-3.5-Turbo를 사용하세요 (20배 저렴함)
export AZURE_OPENAI_MODEL=gpt-35-turbo
...
### 문제: "액세스 거부(Access Denied)" 오류

**증상:** API 호출 시 403 Forbidden 발생

**해결 방법:**  
```bash
# 1. OpenAI 접근 권한이 승인되었는지 확인
...
### 문제: "속도 제한 초과(Rate Limit Exceeded)"
...
# 2. 할당량 증액 요청 (필요한 경우)
# Azure 포털 → OpenAI 리소스 → 할당량 → 증액 요청으로 이동

# 3. 재시도 로직 구현 (이미 chat.py에 있음)
# 애플리케이션이 자동으로 지수 백오프로 재시도함
```

### 문제: "모델을 찾을 수 없음(Model Not Found)"
...
```bash
# 프로덕션 배포의 경우, 관리되는 아이덴티티를 사용하세요
# API 키 대신 사용 (Azure에서 앱 호스팅 필요)

# infra/openai.bicep 파일을 다음과 같이 업데이트하세요:
...
1. **웹 인터페이스 추가** - React/Vue 프런트엔드 구축  
   ```bash
   # azure.yaml에 프런트엔드 서비스를 추가하세요
   # Azure 정적 웹 앱에 배포하세요
   ```

2. **RAG 구현** - Azure AI 검색을 활용한 문서 검색 추가  
   ```python
   # Azure AI 검색 통합
   # 문서 업로드 및 벡터 인덱스 생성
   ```
...
- **[Retail Multi-Agent](../retail-scenario.md)** - 고급 다중 에이전트 아키텍처  
- **[Database App](../../../../examples/database-app)** - 영구 저장소 추가  
- **[Container Apps](../../../../examples/container-app)** - 컨테이너화된 서비스로 배포  

### 학습 자료

- 📚 [AZD 초보자 과정](../../README.md) - 메인 코스 홈  
- 📚 [Microsoft Foundry Models 문서](https://learn.microsoft.com/azure/ai-services/openai/) - 공식 문서  
- 📚 [OpenAI API 참조](https://platform.openai.com/docs/api-reference) - API 상세 정보  
- 📚 [책임감 있는 AI](https://www.microsoft.com/ai/responsible-ai) - 모범 사례  
...
- **[채팅 완료 기능](https://learn.microsoft.com/azure/ai-services/openai/how-to/chatgpt)** - 채팅 앱 구축  
...
- **[토큰 계산기](https://platform.openai.com/tokenizer)** - 토큰 사용량 예측  

### 커뮤니티
- **[Azure AI Discord](https://discord.gg/azure)** - 커뮤니티 지원  
...
**🎉 성공!** Microsoft Foundry Models를 배포하고 작동하는 채팅 애플리케이션을 구축했습니다. gpt-4.1의 기능을 탐색하고 다양한 프롬프트와 사용 사례를 실험해 보세요.

**문의사항?** [이슈 열기](https://github.com/microsoft/AZD-for-beginners/issues) 또는 [FAQ](../../resources/faq.md) 확인

**비용 알림:** 테스트 후에는 `azd down`을 실행하여 지속 요금 발생을 방지하세요(활성 사용 시 약 $50-100/월).
