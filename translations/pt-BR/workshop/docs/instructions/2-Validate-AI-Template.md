...
!!! tip "AO FINAL DESTE MÓDULO VOCÊ SERÁ CAPAZ DE"
    ...
    - [ ] **Laboratório 2:** Implantar e Validar o template AI Agents
...
O [Azure Developer CLI](https://learn.microsoft.com/en-us/azure/developer/azure-developer-cli/) ou `azd` é uma ferramenta de linha de comando open-source que simplifica o fluxo de trabalho do desenvolvedor ao criar e implantar aplicações no Azure.

[AZD Templates](https://learn.microsoft.com/azure/developer/azure-developer-cli/azd-templates) são repositórios padronizados que incluem código de aplicação de exemplo, ativos de _infrastructure-as-code_ e arquivos de configuração do `azd` para uma arquitetura de solução coesa. Provisionar a infraestrutura torna-se tão simples quanto um comando `azd provision` — enquanto usar `azd up` permite que você provisionque a infraestrutura **e** implante sua aplicação em uma única etapa!

Como resultado, iniciar seu processo de desenvolvimento de aplicação pode ser tão simples quanto encontrar o _AZD Starter template_ certo que mais se aproxime das necessidades da sua aplicação e infraestrutura — e então customizar o repositório para atender aos requisitos do seu cenário.
...
![Escolha](../../../../../translated_images/pt-BR/01-pick-template.60d2d5fff5ebc374.webp)
...
1. [X] [Get Started with AI Agents](https://github.com/Azure-Samples/get-started-with-ai-agents) que também implanta um AI Agent padrão (com os Foundry Agents). Use isto para se familiarizar com soluções agentic de IA envolvendo ferramentas e modelos.

Visite o segundo link em uma nova aba do navegador (ou clique em `Open in GitHub` no cartão relacionado). Você deverá ver o repositório para este AZD Template. Reserve um minuto para explorar o README. A arquitetura da aplicação é assim:
...
Vamos tentar implantar este template e garantir que ele seja válido. Seguiremos as diretrizes na seção de [Getting Started](https://github.com/Azure-Samples/get-started-with-ai-agents?tab=readme-ov-file#getting-started).
...
1. Escolha um ambiente de trabalho para o repositório do template:
      ...
      - **Clone local ou dev container**: Clone `Azure-Samples/get-started-with-ai-agents` e abra-o no VS Code
...
Complete as etapas do fluxo de trabalho que isto irá acionar:
...
1. Você será solicitado a escolher uma região - use `East US 2`

Agora, aguarde a conclusão do provisionamento. **Isto leva 10-15 minutos**

1. Quando terminar, seu console mostrará uma mensagem de SUCCESS como esta:
      ```bash title="" linenums="0"
...
1. **Você agora está pronto para validar a infraestrutura e a aplicação implantadas**.
...
1. Tente fazer algumas [perguntas de exemplo](https://github.com/Azure-Samples/get-started-with-ai-agents/blob/main/docs/sample_questions.md)
      ...
      1. Pergunte: ```Qual a melhor barraca abaixo de $200 para duas pessoas, e quais recursos ela inclui?```
...
1. Retorne à página _Overview_ do seu grupo de recursos no Azure Portal
...
1. Você deverá ver a página do Foundry Project para sua aplicação de IA
...
1. Selecione-o - e você verá os detalhes do Agent. Observe o seguinte:
      ...
      - o agente `Knowledge` indica que tem 32 arquivos carregados (para file search)
...
1. Procure a opção `Data+indexes` no menu à esquerda e clique para ver os detalhes. 
      ...
      - Estes corresponderão aos 12 arquivos de cliente e 20 arquivos de produto em `src/files`
...
Retorne à aba do navegador com o endpoint do Container App e pergunte: `Que produtos Amanda Perez possui?`. Você deverá ver algo assim:
...
Vamos desenvolver um pouco mais de intuição sobre as capacidades do Microsoft Foundry, experimentando o Agent no Agents Playground.
...
1. Faça a mesma pergunta: `Que produtos Amanda Perez possui?`

    ![Dados](../../../../../translated_images/pt-BR/09-ask-in-playground.a1b93794f78fa676.webp)

Você obtém a mesma (ou similar) resposta - mas você também obtém informações adicionais que podem ser usadas para entender a qualidade, custo e performance de sua aplicação agentic. Por exemplo:

1. Observe que a resposta cita arquivos de dados usados para "fundamentar" a resposta
1. Passe o cursor sobre quaisquer destes rótulos de arquivo - os dados correspondem à sua consulta e à resposta exibida?

Você também vê uma linha de _estatísticas_ abaixo da resposta.
...
1. Clique na aba `Metadata` para ver atributos adicionais da execução, que podem fornecer contexto útil para depuração posterior.   

      ![Agente](../../../../../translated_images/pt-BR/11-view-run-info-metadata.7966986122c7c2df.webp)


1. Clique na aba `Evaluations` para ver autoavaliações feitas sobre a resposta do agente. Estas incluem avaliações de segurança (por exemplo, Self-harm) e avaliações específicas do agente (por exemplo, resolução de intenção, adesão à tarefa).

      ![Agente](../../../../../translated_images/pt-BR/12-view-run-info-evaluations.ef25e4577d70efeb.webp)

1. Por fim, clique na aba `Monitoring` no menu da barra lateral.
...
Até agora, percorremos a implantação no navegador - e validamos que nossa infraestrutura foi provisionada e a aplicação está operacional. Mas para trabalhar com a aplicação de forma _code-first_, precisamos configurar nosso ambiente de desenvolvimento local com as variáveis relevantes necessárias para trabalhar com esses recursos. Usar `azd` facilita isso.
...
1. As variáveis de ambiente são armazenadas em `.azure/<env-name>/.env` - isso as escopas para o ambiente `env-name` usado durante a implantação e ajuda a isolar ambientes entre diferentes alvos de implantação no mesmo repositório.
...
1. Defina uma nova variável de ambiente para o `azd`. Aqui, atualizamos o nome do modelo do agente. _Observação: quaisquer alterações feitas serão refletidas imediatamente no arquivo `.azure/<env-name>/.env`._
...
1. Observe que alguns recursos são persistentes (por exemplo, implantações de modelo) e requererão mais do que apenas um `azd up` para forçar a reimplantação. Vamos tentar desmontar a implantação original e reimplantar com as variáveis de ambiente alteradas.
...
- [X] Abriu o template em um ambiente de desenvolvimento suportado
