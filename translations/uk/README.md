...
- **Реалізуєте Infrastructure as Code**: Управління ресурсами Azure за допомогою шаблонів Bicep
...
- **Створите багатосервісні рішення**: Розгортання складних AI-архітектур
...
- **⚙️ Налаштування**: [Встановлення та налаштування](docs/getting-started/installation.md) - Гіди для різних платформ
...
**💡 Результат розділу**: Успішно розгорніть простий веб-додаток на Azure за допомогою AZD
...
- **🛠️ Лабораторія**: [Лабораторія AI Workshop](docs/ai-foundry/ai-workshop-lab.md) - Підготовка ваших AI-рішень до AZD
- **🎥 Інтерактивний гід**: [Матеріали лабораторії](workshop/README.md) - Навчання через браузер з MkDocs * DevContainer Environment
...
#### Ресурси для навчання
- **📖 Конфігурація**: [Гід з конфігурації](docs/getting-started/configuration.md) - Налаштування середовища
...
- Налаштуйте автентифікацію через керовану ідентичність
- Реалізуйте конфігурації, специфічні для середовища

**💡 Результат розділу**: Управляйте кількома середовищами з належною автентифікацією та безпекою
...
#### Ресурси для навчання
- **📖 Розгортання**: [Гід з розгортання](docs/deployment/deployment-guide.md) - Повні робочі процеси
...
**💡 Результат розділу**: Розгорніть складні багатосервісні додатки за допомогою власних шаблонів інфраструктури

---

### 🎯 Розділ 5: Багатосервісні AI-рішення (просунутий рівень)
**Передумови**: Завершені розділи 1-2  
**Тривалість**: 2-3 години  
**Складність**: ⭐⭐⭐⭐

#### Що ви дізнаєтесь
- Шаблони багатосервісної архітектури
- Оркестрація та координація агентів
- Розгортання AI-рішень, готових до продакшн

#### Ресурси для навчання
- **🤖 Представлений проєкт**: [Рішення для роздрібної торгівлі](examples/retail-scenario.md) - Повна реалізація
- **🛠️ ARM-шаблони**: [Пакет ARM-шаблонів](../../examples/retail-multiagent-arm-template) - Розгортання в один клік
- **📖 Архітектура**: Шаблони координації багатосервісних агентів
...
**💡 Результат розділу**: Розгорніть та управляйте готовим до продакшн багатосервісним AI-рішенням з агентами для клієнтів та інвентаризації
...
**💡 Результат розділу**: Перевірте та оптимізуйте розгортання перед виконанням
...
- **🕵️ Налагодження**: [Гід з налагодження](docs/troubleshooting/debugging.md) - Покрокові стратегії
...
- Вирішуйте проблеми автентифікації
- Налагоджуйте підключення до сервісів AI

**💡 Результат розділу**: Самостійно діагностуйте та вирішуйте поширені проблеми розгортання
...
Наші матеріали воркшопу забезпечують структурований, інтерактивний досвід навчання, який доповнює навчальну програму, розділену на глави. Воркшоп розроблений як для самостійного навчання, так і для занять під керівництвом інструктора.

#### 🛠️ Особливості воркшопу
- **Інтерфейс у браузері**: Воркшоп на основі MkDocs з функціями пошуку, копіювання та тем
...
1. **Фаза відкриття** (45 хвилин)
   ...
   - Розуміння шаблонів архітектури з кількома агентами
...
- **Розгортати AI-додатки у виробництві**: Використовувати AZD із сервісами Azure AI Foundry
- **Опанувати архітектури з кількома агентами**: Реалізовувати рішення з координацією AI-агентів
- **Реалізовувати найкращі практики безпеки**: Налаштовувати автентифікацію та контроль доступу
- **Оптимізувати для масштабування**: Проєктувати економічно ефективні та продуктивні розгортання
- **Усувати проблеми з розгортанням**: Самостійно вирішувати поширені проблеми
...
- **Інтегровані робочі процеси** - Безперебійне забезпечення, розгортання та моніторинг додатків
...
- **Повні AI-робочі процеси** - Від розробки моделі до розгортання у виробництві з належним моніторингом
- **Оптимізація витрат** - Розумне розподілення ресурсів і стратегії масштабування для AI-навантажень
- **Інтеграція з Azure AI Foundry** - Безперебійне підключення до каталогу моделей і кінцевих точок AI Foundry

---

## 🎯 Бібліотека шаблонів і прикладів

### Рекомендовано: Шаблони Azure AI Foundry
...
| [**Автоматизація робочих процесів з кількома агентами**](https://github.com/Azure-Samples/get-started-with-ai-chat) | Розділ 5 | ⭐⭐⭐ | AzureOpenAI + Azure AI Agent Service + Semantic Kernel + Azure CosmosDB + Azure Container Apps|
...
| [**retail-multi-agent-solution**](examples/retail-scenario.md) | Розділ 5 | ⭐⭐⭐⭐ | Архітектура з кількома агентами для клієнтів і інвентаризації |
...
- [Статичний вебсайт](../../examples/static-site) - Розгортання статичного контенту
- [Базовий API](../../examples/basic-api) - Розгортання REST API

#### Інтеграція з базами даних (Розділи 3-4)  
- [Додаток із базою даних](../../examples/database-app) - Шаблони підключення до бази даних
- [Обробка даних](../../examples/data-processing) - Розгортання ETL-робочого процесу

#### Складні шаблони (Розділи 4-8)
- [Додатки в контейнерах](../../examples/container-app) - Розгортання в контейнерах
- [Мікросервіси](../../examples/microservices) - Багатосервісні архітектури  
...
- [**Воркшоп для початківців з AZD**](workshop/README.md) - Повні матеріали для практичного воркшопу з інтеграцією GitHub Codespaces
...
- [ ] **Розділ 6**: Перевірка перед розгортанням і планування ✅
- [ ] **Розділ 7**: Усунення несправностей і налагодження ✅
...
- **Професійні навички**: Можливості розгортання, готові до використання в підприємствах  
- **Визнання в спільноті**: Активний учасник спільноти розробників Azure
- **Кар'єрний ріст**: Затребувані навички AZD і розгортання AI
...
- **Технічні проблем
- Сприяти впровадженню найкращих практик для розгортання AI
- Впливати на розробку майбутніх функцій AI + AZD

### Внесок у курс
Ми раді вашим внескам! Будь ласка, ознайомтеся з нашим [Посібником для внесків](CONTRIBUTING.md) для отримання деталей щодо:
- **Покращення контенту**: Удосконалення існуючих розділів і прикладів
- **Нових прикладів**: Додавання реальних сценаріїв і шаблонів  
- **Перекладу**: Допомога в підтримці багатомовної підтримки
- **Звітування про помилки**: Підвищення точності та ясності
- **Стандартів спільноти**: Дотримання наших інклюзивних правил спільноти
