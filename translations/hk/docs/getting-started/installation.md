...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T16:46:51+00:00",
...
這份全面的指南將帶您逐步完成在系統上安裝和配置 Azure Developer CLI (azd) 的過程。您將學習針對不同操作系統的多種安裝方法、身份驗證設定，以及初始配置，為 Azure 部署準備您的開發環境。
...
這份指南將協助您在任何操作系統或開發環境中安裝和配置 Azure Developer CLI。
...
### 環境變數
將以下內容添加至您的 shell 配置檔（如 `.bashrc`、`.zshrc`、`.profile`）：
```bash
# Azure configuration
...
1. **完成身份驗證**：確保您可以訪問 Azure 訂閱
2. **嘗試首次部署**：參考 [首次專案指南](first-project.md)
...
**導航**
- **上一課**：[AZD 基礎](azd-basics.md)
- **下一課**：[配置](configuration.md)

**安裝完成！** 繼續前往 [您的第一個專案](first-project.md) 開始使用 azd 建構專案。
