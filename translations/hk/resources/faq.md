...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T17:48:34+00:00",
...
這份全面的常見問題解答提供了有關 Azure Developer CLI (azd) 和 Azure 部署的最常見問題的答案。快速找到常見問題的解決方案，了解最佳實踐，並釐清 azd 的概念和工作流程。

## 學習目標

通過閱讀這份常見問題解答，您將能夠：
- 快速找到 Azure Developer CLI 常見問題和問題的答案
- 通過實用的問答形式理解關鍵概念和術語
- 獲取常見問題和錯誤場景的故障排除解決方案
- 通過常見的優化問題學習最佳實踐
- 通過專家級問題探索高級功能和能力
- 高效參考成本、安全性和部署策略指導
...
**答**：Azure Developer CLI (azd) 是一個以開發者為中心的命令行工具，可加速將應用程式從本地開發環境部署到 Azure 的時間。它通過模板提供最佳實踐，並幫助完成整個部署生命周期。

### 問：azd 與 Azure CLI 有什麼不同？
**答**：
- **Azure CLI**：通用工具，用於管理 Azure 資源
- **azd**：以開發者為重點的工具，用於應用程式部署工作流程
- azd 內部使用 Azure CLI，但為常見的開發場景提供更高層次的抽象
- azd 包括模板、環境管理和部署自動化

### 問：使用 azd 是否需要安裝 Azure CLI？
**答**：是的，azd 需要 Azure CLI 進行身份驗證和某些操作。請先安裝 Azure CLI，然後再安裝 azd。
...
**答**：
1. 從現有模板結構開始
2. 修改 `azure.yaml`、基礎設施文件和應用程式代碼
3. 使用 `azd up` 進行全面測試
4. 在 GitHub 上發布並添加適當的標籤

### 問：我可以在沒有模板的情況下使用 azd 嗎？
**答**：可以，使用 `azd init` 在現有項目中創建必要的配置文件。您需要手動配置 `azure.yaml` 和基礎設施文件。
...
### 問：我可以部署到多個地區嗎？
**答**：可以，在您的 Bicep 模板中配置多個地區，並為每個環境適當設置 location 參數。
...
### 問：我可以僅部署基礎設施而不部署應用程式嗎？
**答**：可以，使用 `azd provision` 只部署模板中定義的基礎設施組件。
...
**答**：目前，azd 主要支持 Bicep/ARM 模板。Terraform 尚未正式支持，但可能存在社群解決方案。
...
**答**：使用 `azd env new <environment-name>` 創建單獨的環境，並為每個環境配置不同的設置：
```bash
...
2. 檢查 Azure Portal 的部署歷史記錄
3. 查看 Azure Portal 的活動日誌
...
**答**：檢查：
1. 部署是否成功完成
2. 應用程式是否正在運行（在 Azure Portal 中檢查日誌）
3. 網絡安全組是否允許流量
4. DNS/自定義域名是否正確配置
...
**答**：可以，azd 與任何 CI/CD 系統兼容。創建使用 azd 命令的 Azure DevOps 管道。

### 問：如何為 azd 或社群模板做出貢獻？
**答**：
1. **azd 工具**：貢獻到 [Azure/azure-dev](https://github.com/Azure/azure-dev)
2. **模板**：按照 [模板指南](https://github.com/Azure-Samples/awesome-azd) 創建模板
3. **文檔**：貢獻到 [MicrosoftDocs/azure-dev-docs](https://github.com/MicrosoftDocs/azure-dev-docs)
...
### **回報問題**
- [GitHub 問題](https://github.com/Azure/azure-dev/issues/new) - 錯誤回報及功能請求  
- 請包含相關日誌、錯誤訊息及重現步驟  
