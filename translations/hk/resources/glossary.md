...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T18:03:44+00:00",
...
這份全面的詞彙表提供了 Azure Developer CLI 和 Azure 雲端開發中使用的術語、概念和縮寫的定義。是理解技術文檔、解決問題以及有效溝通 azd 專案和 Azure 服務的重要參考。
...
**ARM 模板**  
Azure Resource Manager 模板。基於 JSON 的基礎架構即代碼格式，用於以聲明方式定義和部署 Azure 資源。
...
以開發者為中心的命令行工具，通過模板和基礎架構即代碼加速構建和部署 Azure 應用程式的過程。
...
**Resource Token**  
由 azd 生成的唯一字符串，用於確保資源名稱在部署中是唯一的。
...
**Storage Account**  
Azure 的服務，提供包括 Blob、文件、隊列和表的雲存儲。
...
**YAML**  
YAML Ain't Markup Language。用於配置文件（例如 azure.yaml）的可讀性高的數據序列化標準。
...
| KQL | Kusto Query Language | Azure 數據服務的查詢語言 |
...
| SQL | Structured Query Language | 管理關係型數據庫的語言 |
