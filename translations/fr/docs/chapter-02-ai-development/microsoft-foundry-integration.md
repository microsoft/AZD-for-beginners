...
- **Prompt Flow** : Concepteur visuel pour workflows IA
...
- **Sécurité et sûreté** : Fonctionnalités IA responsables intégrées

## AZD + Microsoft Foundry : Mieux ensemble

| Fonctionnalité | Microsoft Foundry | Avantage de l'intégration AZD |
|---------|-----------------|------------------------|
...
| **Gestion d'environnement** | Focus sur un seul environnement | Multi-environnements (dev/staging/prod) |
...
- Familiarité de base avec Microsoft Foundry

> **Base actuelle AZD :** Ces exemples ont été testés avec `azd` `1.27.1`. Pour le workflow agent IA, utilisez la dernière version preview de l'extension et vérifiez la version installée avant de commencer.

## Modèles d'intégration principaux

### Modèle 1 : Intégration Microsoft Foundry Models

**Cas d'utilisation** : Déployer des applications de chat avec modèles Microsoft Foundry Models
...
### Modèle 2 : Intégration AI Search + RAG
...
**Cas d'utilisation** : Workflows de traitement et analyse de documents
...
AZD fournit des extensions qui ajoutent des fonctionnalités spécifiques à l'IA pour travailler avec les services Microsoft Foundry :
...
Les extensions IA évoluent rapidement en preview. Si une commande fonctionne différemment de ce qui est montré ici, mettez à jour l'extension concernée avant de dépanner le projet lui-même.

### Déploiement Agent-First avec `azd ai`

Si vous avez un manifeste agent, utilisez `azd ai agent init` pour générer un projet connecté au service Foundry Agent :
...
Les versions preview récentes de `azure.ai.agents` ont également ajouté le support d'initialisation basée sur des modèles pour `azd ai agent init`. Si vous suivez des exemples d'agents plus récents, consultez l'aide de l'extension pour les flags exacts disponibles dans votre version installée.
...
# Pour les déploiements d'applications d'IA à longue durée dans azd 1.23.11+
...
**Symptômes :**
- Erreurs 401/403 lors de l’appel aux services IA
...
- Versions de modèles spécifiques en échec
...
**Description** : L'exemple Azure AI le plus populaire — une application de chat RAG prête pour la production qui vous permet de poser des questions sur vos propres documents. Utilise GPT-4.1-mini pour le chat, text-embedding-3-large pour les embeddings, et Azure AI Search pour la récupération. Supporte les documents multimodaux, l'entrée/sortie vocale, l’authentification Microsoft Entra, et le traçage Application Insights.
...
**Description** : Équivalent .NET/C# de l'exemple de chat RAG en Python. Construit avec ASP.NET Core Minimal API et frontend Blazor WebAssembly. Inclut chat vocal, support vision GPT-4o-mini, et un client hybride desktop/mobile .NET MAUI Blazor compagnon.
...
**Description** : Version Java de l'exemple de chat RAG utilisant Langchain4J pour l’orchestration IA. Supporte architecture microservices pilotée par événements, multiples stratégies de recherche (texte, vecteur, hybride), téléchargement de documents avec Azure Document Intelligence, et déploiement sur Azure Container Apps ou Azure Kubernetes Service.
...
### Copilote commerce de détail Enterprise avec Microsoft Foundry
...
**Description** : Copilote RAG commerce de détail complet utilisant Microsoft Foundry et Prompty. Un chatbot du magasin Contoso Outdoor qui fonde ses réponses sur le catalogue produits et les données des commandes clients. Démontre le workflow complet GenAIOps — prototypage avec Prompty, évaluation assistée par IA, et déploiement via AZD sur Container Apps.
...
### Application multi-agents d’écriture créative
...
**Description** : Application de référence pour l’orchestration IA multi-agent utilisant trois frameworks (LangChain.js, LlamaIndex.TS et Microsoft Agent Framework). Dispose de serveurs MCP (Model Context Protocol) en quatre langages déployés en Azure Container Apps serverless avec monitoring OpenTelemetry.
...
**Description** : Template Bicep minimal qui déploie les services Azure AI avec modèles machine learning configurés. Point de départ léger quand vous avez seulement besoin de l’infrastructure Azure AI provisionnée sans pile applicative complète.
...
> **Consultez plus de templates** : Visitez la [Galerie Awesome AZD AI Template](https://azure.github.io/awesome-azd/?tags=ai) pour plus de 80 templates AZD spécifiques à l’IA couvrant langages et scénarios.

## Étapes suivantes

1. **Essayez les exemples** : Commencez avec un template pré-construit qui correspond à votre cas d’usage
...
4. **Optimisez les coûts** : Ajustez les configurations pour votre budget
5. **Sécurisez votre déploiement** : Implémentez des modèles de sécurité d’entreprise
6. **Passez en production** : Ajoutez fonctions multi-régions et haute disponibilité

## 🎯 Exercices pratiques

### Exercice 1 : Déployer l’application de chat Microsoft Foundry Models (30 minutes)
...
- [ ] Peut poser des questions et obtenir des réponses IA
- [ ] Application Insights affiche les données de télémétrie
- [ ] Nettoyage réussi des ressources

**Coût estimé** : 5 à 10 $ pour 30 minutes de test
...
- [ ] Différents réglages de capacité appliqués
- [ ] Modèles accessibles via API
- [ ] Peut appeler les deux modèles depuis l’application
...
- [ ] Peut consulter les données de coût dans Azure Portal
- [ ] Seuils budgétaires adéquatement fixés

## 💡 Questions fréquentes

<details>
<summary><strong>Comment réduire les coûts Microsoft Foundry Models en développement ?</strong></summary>

1. **Utiliser le niveau gratuit** : Microsoft Foundry Models offre 50 000 tokens/mois gratuits
2. **Réduire la capacité** : Réglez la capacité à 10 TPM au lieu de 30+ pour le dev
3. **Utilisez azd down** : Libérez les ressources quand vous ne développez pas activement
4. **Cachez les réponses** : Implémentez un cache Redis pour les requêtes répétées
5. **Utilisez l’ingénierie de prompt** : Réduisez l’usage des tokens avec des prompts efficaces
...
<details>
<summary><strong>Quelle est la différence entre les Microsoft Foundry Models et l'API OpenAI ?</strong></summary>

**Microsoft Foundry Models** :
- Sécurité et conformité entreprise
- Intégration réseau privée
- Garanties SLA
- Authentification avec identité managée
...
# Aller dans le portail Azure > Quotas > Demander une augmentation
```
</details>
...
Consultez le modèle [azure-search-openai-demo](https://github.com/Azure-Samples/azure-search-openai-demo).
</details>
...
4. Mettez en place une limitation de débit
...
- **Agent Skills** : [Compétence Microsoft Foundry sur skills.sh](https://skills.sh/microsoft/github-copilot-for-azure/microsoft-foundry) - Installez les compétences agent Azure + Foundry dans votre éditeur avec `npx skills add microsoft/github-copilot-for-azure`
...
- **📖 Chapitre actuel** : Chapitre 2 - Développement AI-First
- **⬅️ Chapitre précédent** : [Chapitre 1 : Votre premier projet](../chapter-01-foundation/first-project.md)
- **➡️ Suivant** : [Déploiement des modèles AI](ai-model-deployment.md)
- **🚀 Chapitre suivant** : [Chapitre 3 : Configuration](../chapter-03-configuration/configuration.md)
