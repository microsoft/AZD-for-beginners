...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2026-01-01T12:13:59+00:00",
...
![AZD 初學者](../../translated_images/tw/azdbeginners.5527441dd9f74068.webp) 
...
1. **Fork 本儲存庫**: Click [![GitHub 分叉](https://img.shields.io/github/forks/microsoft/azd-for-beginners.svg?style=social&label=Fork)](https://GitHub.com/microsoft/azd-for-beginners/fork)
2. **Clone 本儲存庫**: `git clone https://github.com/microsoft/azd-for-beginners.git`
3. **加入社群**: [Azure Discord Communities](https://discord.com/invite/ByRwuEEgH4) 尋求專家支援
4. **選擇您的學習路徑**: 從下方選擇符合您經驗程度的章節
...
透過設計成循序漸進章節的教學內容，掌握 Azure Developer CLI (azd)。**特別著重於結合 Microsoft Foundry 的 AI 應用部署。**

### 為什麼本課程對現代開發者至關重要

根據 Microsoft Foundry Discord 社群洞察，**45% 的開發者希望使用 AZD 進行 AI 工作負載**，但會面臨以下挑戰：
- 複雜的多服務 AI 架構
- 生產環境 AI 部署最佳實務  
- Azure AI 服務的整合與設定
- AI 工作負載的成本優化
...
- **實作基礎架構即程式碼**：以 Bicep 範本管理 Azure 資源
- **排除部署問題**：解決常見問題並進行除錯
- **為生產環境最佳化**：安全性、擴展、監控與成本管理
...
**💡 章節成果**：使用 AZD 成功將簡單的 Web 應用部署至 Azure
...
- **🛠️ 工作坊**: [AI 工作坊實驗室](docs/microsoft-foundry/ai-workshop-lab.md) - 讓您的 AI 解決方案符合 AZD 準備
- **🎥 互動式指南**: [工作坊教材](workshop/README.md) - 以瀏覽器為基礎的學習，使用 MkDocs * DevContainer 環境
...
**💡 章節成果**：部署並設定具備 RAG 能力的 AI 驅動聊天應用
...
**📊 時間投入：** 1-2 小時  
**📈 完成後技能等級：** 可部署並設定生產就緒的 AI 應用程式  
**💰 成本意識：** 了解開發環境每月約 $80-150，生產環境每月約 $300-3500

#### 💰 AI 部署的成本考量

**開發環境（估計每月 $80-150）：**
- Azure OpenAI（按使用付費）：$0-50/月（依據 token 使用量）
- AI Search（基礎層）：$75/月
- Container Apps（消耗模式）：$0-20/月
- 儲存（標準）：$1-5/月

**生產環境（估計每月 $300-3,500+）：**
- Azure OpenAI（PTU 確保穩定效能）：$3,000+/月 或 高流量時按使用付費
- AI Search（標準層）：$250/月
...
**💡 成本優化建議：**
- 學習階段使用 **Free Tier** 的 Azure OpenAI（包含每月 50,000 tokens）
- 不開發時執行 `azd down` 以解除配置資源
- 以消耗計費為起點，僅在生產環境才升級到 PTU
...
- **🔐 安全**: [驗證模式與管理識別](docs/getting-started/authsecurity.md) - 驗證模式
...
- **🏗️ 佈署**: [資源佈署](docs/deployment/provisioning.md) - Azure 資源管理
...
**💡 章節成果**: 部署並管理具生產能力的多代理 AI 解決方案，包含 Customer 與 Inventory 代理
...
#### 學習資源
- **🔧 常見問題**: [常見問題](docs/troubleshooting/common-issues.md) - 常見問答與解答
...
- **📊 監控**: [Application Insights 集成](docs/pre-deployment/application-insights.md) - 監控
...
> **⚠️ 工作坊狀態：開發中**  
> 工作坊教材目前正在開發與精進。核心模組已可運作，但部分進階章節尚未完成。我們正積極補全所有內容。 [追蹤進度 →](workshop/README.md)

### 互動工作坊教材
**以瀏覽器為基礎工具與引導式練習提供完整的實作式學習**

我們的工作坊教材提供結構化、互動式的學習體驗，補充上述章節式課程。該工作坊設計適合自學與講師帶領的課程。

#### 🛠️ 工作坊特色
- **以瀏覽器為基礎的介面**: 完整的 MkDocs 驅動工作坊，具搜尋、複製與主題功能
- **GitHub Codespaces 整合**: 一鍵設定開發環境
- **結構化學習路徑**: 7 步引導式練習（總計 3.5 小時）
- **探索 → 部署 → 客製化**: 漸進式方法
...
> **注意：** 這些範本示範各種 AI 模式。有些為外部的 Azure Samples，其他為本地實作。
...
| 範本 | 學習章節 | 複雜度 | 主要學習重點 |
|----------|------------------|------------|--------------|
| [**openai-chat-app-quickstart**](https://github.com/Azure-Samples/openai-chat-app-quickstart) | 第2章 | ⭐ | AI 基本部署範式 |
...
| [**retail-multi-agent-solution**](examples/retail-scenario.md) | 第5章 | ⭐⭐⭐⭐ | 具 Customer 與 Inventory 代理的多代理架構 |
...
#### 本地範例 - 容器應用（第2-5章）
**此儲存庫中的完整容器部署範例：**
- [**容器應用範例**](examples/container-app/README.md) - 完整的容器化部署指南
  - [Simple Flask API](../../examples/container-app/simple-flask-api) - 基本 REST API，具可縮減為零
...
### 外部範本集合
- [**官方 AZD 範本畫廊**](https://azure.github.io/awesome-azd/) - 官方與社群範本的精選集
...
- [**Examples 目錄**](examples/README.md) - 本地學習範例與詳細說明
...
### 快速參考
- [**命令速查表**](resources/cheat-sheet.md) - 必要的 azd 命令，按章節整理
- [**詞彙表**](resources/glossary.md) - Azure 與 azd 術語  
- [**常見問題（FAQ）**](resources/faq.md) - 按學習章節分類的常見問題
- [**學習指南**](resources/study-guide.md) - 全面的練習題

### 實作工作坊
- [**AI 工作坊實驗室**](docs/microsoft-foundry/ai-workshop-lab.md) - 使您的 AI 解決方案可使用 AZD 部署 (2-3 hours)
- [**互動工作坊指南**](workshop/README.md) - 基於瀏覽器的工作坊，使用 MkDocs 和 DevContainer 環境
- [**Structured Learning Path**](../../workshop/docs/instructions) -7 步驟引導練習 (Discovery → Deployment → Customization)
- [**AZD 新手工作坊**](workshop/README.md) - 完整的實作工作坊教材，整合 GitHub Codespaces
...
- **取得幫助：** [Azure Discord](https://discord.gg/microsoft-azure) #azure-developer-cli
...
### 學習驗證
完成每個章節後，透過下列方式驗證您的知識：
1. **實作練習**：完成該章節的動手部署
2. **知識檢核**：查看該章節的常見問題（FAQ）
...
- **專業技能**：具備企業就緒的部署能力  
- **社群認可**：成為 Azure 開發者社群的活躍會員
- **職涯進展**：受需求青睞的 AZD 與 AI 部署專長
...
### 尋求協助與支援
- **技術問題**： [報告錯誤與功能請求](https://github.com/microsoft/azd-for-beginners/issues)
- **學習疑問**： [Microsoft Azure Discord 社群](https://discord.gg/microsoft-azure) and [![Microsoft Foundry Discord](https://dcbadge.limes.pink/api/server/nTYy5BXMWG)](https://discord.gg/nTYy5BXMWG)
...
免責聲明：
本文件是使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 所翻譯。雖然我們努力確保內容的準確性，但請注意，自動翻譯可能仍包含錯誤或不準確之處。原始語言的文件應視為具權威性的版本。對於重要資訊，建議採用專業人工翻譯。我們不對因使用本翻譯而產生的任何誤解或曲解承擔任何責任。
