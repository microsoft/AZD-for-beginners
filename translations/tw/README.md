...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-19T15:43:55+00:00",
...
- **🛠️ 實作**：[您的第一個專案](docs/getting-started/first-project.md) - 步驟式教程
...
**💡 章節成果**：使用 AZD 成功部署一個簡單的網頁應用至 Azure
...
**💡 章節成果**：部署並配置具有 RAG 功能的 AI 驅動聊天應用
...
**📊 時間投入：** 1-2 小時  
**📈 技能水平提升：** 能夠部署並配置生產就緒的 AI 應用  
**💰 成本意識：** 了解 $80-150/月開發成本，$300-3500/月生產成本
...
- **🕵️ 除錯**：[除錯指南](docs/troubleshooting/debugging.md) - 步驟分解策略
...
> **⚠️ 工作坊狀態：開發中**  
> 工作坊材料目前正在開發與完善中。核心模組已可使用，但部分進階章節尚未完成。我們正積極努力完成所有內容。[追蹤進度 →](workshop/README.md)

### 互動式工作坊材料
**透過瀏覽器工具與指導練習，提供全面的實作學習體驗**

我們的工作坊材料提供結構化的互動學習體驗，補充上述章節式課程。工作坊適合自學與講師指導的課程。

#### 🛠️ 工作坊特色
- **基於瀏覽器的介面**：完整的 MkDocs 驅動工作坊，具備搜尋、複製與主題功能
- **GitHub Codespaces 整合**：一鍵設置開發環境
- **結構化學習路徑**：7 步驟指導練習（總時長 3.5 小時）
- **探索 → 部署 → 客製化**：漸進式方法
- **互動式 DevContainer 環境**：預配置工具與依賴項

#### 📚 工作坊結構
工作坊遵循 **探索 → 部署 → 客製化** 的方法：

1. **探索階段**（45 分鐘）
   - 探索 Microsoft Foundry 模板與服務
   - 理解多代理架構模式
   - 審查部署需求與先決條件
...
# 點擊 "Code" → "Create codespace on main" 在儲存庫中
...
#### 🎯 工作坊學習成果
完成工作坊後，參與者將能：
- **部署生產 AI 應用程式**：使用 AZD 與 Microsoft Foundry 服務
...
- **🛠️ AI 工作坊實驗室**：[AI 工作坊實驗室](docs/microsoft-foundry/ai-workshop-lab.md) - 專注於 AI 的練習
...
> **注意：** 這些模板展示了各種 AI 模式。有些是外部 Azure 範例，有些是本地實作。

| 模板 | 章節 | 難度 | 服務 | 類型 |
|------|------|------|------|------|
| [**AI 聊天入門**](https://github.com/Azure-Samples/get-started-with-ai-chat) | 第二章 | ⭐⭐ | AzureOpenAI + Azure AI 模型推理 API + Azure AI 搜索 + Azure 容器應用 + Application Insights | 外部 |
| [**AI 代理入門**](https://github.com/Azure-Samples/get-started-with-ai-agents) | 第二章 | ⭐⭐ | Azure AI 代理服務 + AzureOpenAI + Azure AI 搜索 + Azure 容器應用 + Application Insights | 外部 |
| [**Azure 搜索 + OpenAI 範例**](https://github.com/Azure-Samples/azure-search-openai-demo) | 第二章 | ⭐⭐ | AzureOpenAI + Azure AI 搜索 + 應用服務 + 儲存 | 外部 |
...
| [**Contoso Chat RAG**](https://github.com/Azure-Samples/contoso-chat) | 第八章 | ⭐⭐⭐⭐ | AzureOpenAI + AI 搜索 + Cosmos DB + 容器應用 | 外部 |
| [**零售多代理解決方案**](examples/retail-scenario.md) | 第五章 | ⭐⭐⭐⭐ | AzureOpenAI + AI 搜索 + 儲存 + 容器應用 + Cosmos DB | **本地** |
...
| [**azure-search-openai-demo**](https://github.com/Azure-Samples/azure-search-openai-demo) | 第二章 | ⭐⭐ | 使用 Azure AI 搜索的 RAG 實作 |
| [**ai-document-processing**](https://github.com/Azure-Samples/ai-document-processing) | 第四章 | ⭐⭐ | 文件智能整合 |
| [**agent-openai-python-prompty**](https://github.com/Azure-Samples/agent-openai-python-prompty) | 第五章 | ⭐⭐⭐ | 代理框架與函數調用 |
| [**contoso-chat**](https://github.com/Azure-Samples/contoso-chat) | 第八章 | ⭐⭐⭐ | 企業 AI 編排 |
| [**retail-multi-agent-solution**](examples/retail-scenario.md) | 第五章 | ⭐⭐⭐⭐ | 客戶與庫存代理的多代理架構 |

### 按範例類型學習

> **📌 本地與外部範例：**  
> **本地範例**（此存儲庫中）= 可立即使用  
> **外部範例**（Azure 範例）= 從連結的存儲庫克隆

#### 本地範例（可立即使用）
- [**零售多代理解決方案**](examples/retail-scenario.md) - 使用 ARM 模板的完整生產就緒實作
  - 多代理架構（客戶 + 庫存代理）
  - 全面監控與評估
  - 一鍵部署透過 ARM 模板
...
- [**容器應用範例**](examples/container-app/README.md) - 容器化部署的完整指南
  - [簡單 Flask API](../../examples/container-app/simple-flask-api) - 具備零負載縮減的基本 REST API
...
### 完成課程的好處
完成所有章節後，您將擁有：
- **實務經驗**：將真實的 AI 應用程式部署到 Azure
- **專業技能**：具備企業級部署能力  
- **社群認可**：成為 Azure 開發者社群的活躍成員
- **職涯提升**：掌握 AZD 和 AI 部署的熱門專業技能
...
### 授權
此專案依據 MIT 授權條款授權 - 詳情請參閱 [LICENSE](../../LICENSE) 文件。
