...
![Pick](../../../../../translated_images/tw/01-pick-template.60d2d5fff5ebc374.png)
...
![Arch](../../../../../translated_images/tw/architecture.8cec470ec15c65c7.png)
...
1. 您的 Azure 入口網站現在將有一個以環境名稱命名的資源組：
      ...
      ![Infra](../../../../../translated_images/tw/02-provisioned-infra.46c706b14f56e0bf.png)
...
1. 您應該會看到一個託管的應用程式前端 UI，如下所示：
   ...
   ![App](../../../../../translated_images/tw/03-test-application.471910da12c3038e.png)
...
1. 您應該會得到類似以下的答案。_但這是如何運作的？_
      ...
      ![App](../../../../../translated_images/tw/03-test-question.521c1e863cbaddb6.png)
...
1. 您應該會看到以下內容。點擊 `Go to Azure AI Foundry Portal` 按鈕。
   ![Foundry](../../../../../translated_images/tw/04-view-foundry-project.fb94ca41803f28f3.png)

1. 您應該會看到您的 AI 應用程式的 Foundry 項目頁面
   ![Project](../../../../../translated_images/tw/05-visit-foundry-portal.d734e98135892d7e.png)

1. 點擊 `Agents` - 您將看到項目中佈建的默認代理
   ![Agents](../../../../../translated_images/tw/06-visit-agents.bccb263f77b00a09.png)

1. 選擇它 - 您將看到代理的詳細信息。注意以下內容：

      - 該代理默認使用文件搜索
      - 該代理的 `Knowledge` 表示已上傳 32 個文件（用於文件搜索）
      ![Agents](../../../../../translated_images/tw/07-view-agent-details.0e049f37f61eae62.png)

1. 在左側菜單中找到 `Data+indexes` 選項並點擊查看詳細信息。

      - 您應該會看到上傳的 32 個數據文件。
      - 這些文件對應於 `src/files` 文件夾中的 12 個客戶文件和 20 個產品文件。
      ![Data](../../../../../translated_images/tw/08-visit-data-indexes.5a4cc1686fa0d19a.png)
...
![Data](../../../../../translated_images/tw/09-ask-in-aca.4102297fc465a4d5.png)
...
1. 問同樣的問題：`What products does Amanda Perez own?`
    ...
    ![Data](../../../../../translated_images/tw/09-ask-in-playground.a1b93794f78fa676.png)
...
1. 評估的評級是否符合您對回答安全性水平的直覺？
      ...
      ![Data](../../../../../translated_images/tw/10-view-run-info-meter.6cdb89a0eea5531f.png)
...
1. 點擊 `View Run Info` 按鈕 - 您應該會看到此視圖。這是 [代理追蹤](https://learn.microsoft.com/en-us/azure/ai-foundry/how-to/develop/trace-agents-sdk#view-trace-results-in-the-azure-ai-foundry-agents-playground) 的示例。_您也可以通過點擊頂級菜單中的 Thread Logs 獲得此視圖_。
   ...
   - 理解延遲以及執行中花費時間的地方
      ...
      ![Agent](../../../../../translated_images/tw/10-view-run-info.b20ebd75fef6a1cc.png)

1. 點擊 `Metadata` 標籤以查看運行的其他屬性，這些屬性可能為後續調試問題提供有用的上下文。

      ![Agent](../../../../../translated_images/tw/11-view-run-info-metadata.7966986122c7c2df.png)

1. 點擊 `Evaluations` 標籤以查看對代理回答進行的自動評估。這些包括安全性評估（例如，自我傷害）和代理特定的評估（例如，意圖解析、任務遵守）。

      ![Agent](../../../../../translated_images/tw/12-view-run-info-evaluations.ef25e4577d70efeb.png)
...
1. 最後但同樣重要的是，點擊側邊欄菜單中的 `Monitoring` 標籤。
      ...
      ![Agent](../../../../../translated_images/tw/13-monitoring-resources.5148015f7311807f.png)
