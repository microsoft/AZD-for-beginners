...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T17:47:48+00:00",
...
**答**：Azure Developer CLI (azd) 是一個以開發者為中心的命令列工具，能加速將應用程式從本地開發環境部署到 Azure 的過程。它透過範本提供最佳實踐，並協助完成整個部署生命周期。
...
**答**：使用 `azd auth login`，這將開啟瀏覽器窗口進行 Azure 身份驗證。對於 CI/CD 場景，請使用服務主體或受管理的身份。
...
### 問：我可以部署到多個地區嗎？
**答**：可以，在您的 Bicep 範本中配置多個地區，並為每個環境適當設置 location 參數。
...
**答**：檢查：
1. 部署是否成功完成
2. 應用程式是否正在運行（檢查 Azure 入口網站中的日誌）
3. 網路安全群組是否允許流量
4. DNS/自訂域名是否正確配置
...
**答**：可以，azd 適用於任何 CI/CD 系統。建立使用 azd 命令的 Azure DevOps 管道。
...
3. **文檔**：貢獻至 [MicrosoftDocs/azure-dev-docs](https://github.com/MicrosoftDocs/azure-dev-docs)
