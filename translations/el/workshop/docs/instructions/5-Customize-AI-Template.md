...
!!! tip "ΜΕΧΡΙ ΤΟ ΤΕΛΟΣ ΑΥΤΗΣ ΤΗΣ ΕΝΟΤΗΤΑΣ ΘΑ ΜΠΟΡΕΙΤΕ"
    ...
    - [ ] Εξερευνήσατε τις προεπιλεγμένες δυνατότητες των πρακτόρων AI
    - [ ] Προσθέσατε AI Search με το δικό σας ευρετήριο
    - [ ] Ενεργοποιήσατε και αναλύσατε τις μετρικές Tracing
    - [ ] Εκτελέσατε μια εκτέλεση αξιολόγησης
    - [ ] Εκτελέσατε μια σάρωση red-teaming
    - [ ] **Εργαστήριο 5: Δημιουργήθηκε ένα Σχέδιο Προσαρμογής** 

---

## 5.1 Δυνατότητες Πρακτόρων AI

!!! success "Το ολοκληρώσαμε στο Εργαστήριο 01"

- **Αναζήτηση Αρχείων**: Η ενσωματωμένη αναζήτηση αρχείων του OpenAI για ανάκτηση γνώσης
- **Παραπομπές**: Αυτόματη απόδοση πηγών στις απαντήσεις
- **Προσαρμόσιμες Οδηγίες**: Τροποποίηση της συμπεριφοράς και της προσωπικότητας του πράκτορα
...
## 5.4 [Αξιολόγηση Πρακτόρα](https://github.com/Azure-Samples/get-started-with-ai-agents/blob/main/docs/other_features.md#agent-evaluation)

**Τοπική Αξιολόγηση:**

- Ενσωματωμένοι αξιολογητές για αξιολόγηση ποιότητας
- Προσαρμοσμένα σενάρια αξιολόγησης
- Μετρήσεις αναφοράς απόδοσης

**Συνεχής Παρακολούθηση:**

- Αυτόματη αξιολόγηση των ζωντανών αλληλεπιδράσεων
...
- Αυτοματοποιημένες δοκιμές και αξιολόγηση
- Στατιστικές δοκιμές σύγκρισης
...
- Εναλλακτική βασικής πιστοποίησης για ανάπτυξη



!!! quote "ΣΤΟ ΤΕΛΟΣ ΑΥΤΟΥ ΤΟΥ ΕΡΓΑΣΤΗΡΙΟΥ ΘΑ ΠΡΕΠΕΙ ΝΑ ΕΧΕΤΕ"
    - [ ] Ορίσετε τις απαιτήσεις του σεναρίου σας
    - [ ] Προσαρμοσμένες μεταβλητές env (config)
    - [ ] Προσαρμοσμένες οδηγίες πράκτορα (task)
...
#### **Αναπτύξεις Πρακτόρων:** 
   ...
   - Loyalty Agent: Διαχειρίζεται τα προγράμματα επιβράβευσης και τις προωθητικές ενέργειες πελατών

#### **Αναπτύξεις Μοντέλων:**

   - `gpt-4.1`: Κύριο μοντέλο συνομιλίας
   - `o3`: Μοντέλο συλλογισμού για σύνθετα ερωτήματα
   - `gpt-4.1-nano`: Ελαφρύ μοντέλο για απλές αλληλεπιδράσεις
   - `text-embedding-3-large`: Embeddings υψηλής ποιότητας για αναζήτηση
...
#### **Χαρακτηριστικά:**
   ...
   - Red teaming για έλεγχο ασφάλειας
...
#### 5.6.2.1. Προδιαμόρφωση πριν την Ανάπτυξη
...
# Ορισμός ονόματος περιβάλλοντος
azd env set AZURE_ENV_NAME "retail-ai-agents"

# Διαμόρφωση περιοχής (επιλέξτε με βάση τη διαθεσιμότητα του μοντέλου)
azd env set AZURE_LOCATION "eastus2"

# Ενεργοποίηση όλων των προαιρετικών υπηρεσιών
azd env set USE_APPLICATION_INSIGHTS true
azd env set USE_AZURE_AI_SEARCH_SERVICE true
azd env set ENABLE_AZURE_MONITOR_TRACING true

# Διαμόρφωση κύριου μοντέλου συνομιλίας (gpt-4.1 ως το πιο κοντινό διαθέσιμο στο gpt-4.1)
azd env set AZURE_AI_AGENT_MODEL_NAME "gpt-4.1"
azd env set AZURE_AI_AGENT_MODEL_FORMAT "OpenAI"
azd env set AZURE_AI_AGENT_DEPLOYMENT_NAME "chat-primary"
azd env set AZURE_AI_AGENT_DEPLOYMENT_CAPACITY 150

# Διαμόρφωση μοντέλου ενσωμάτωσης για βελτιωμένη αναζήτηση
azd env set AZURE_AI_EMBED_MODEL_NAME "text-embedding-3-large"
azd env set AZURE_AI_EMBED_DEPLOYMENT_NAME "embeddings-large"
azd env set AZURE_AI_EMBED_DEPLOYMENT_CAPACITY 75

# Ορισμός ονόματος πρακτορα (θα δημιουργήσει τον πρώτο πρακτορα)
azd env set AZURE_AI_AGENT_NAME "shopper-agent"

# Διαμόρφωση ευρετηρίου αναζήτησης
azd env set AZURE_AI_SEARCH_INDEX_NAME "retail-products"
...
# Δοκιμή διαθεσιμότητας της εφαρμογής ιστού
APP_URL=$(azd show --output json | jq -r '.services.api_and_frontend.project.target.url // empty')
if [ ! -z "$APP_URL" ]; then
...
# Εκτέλεση αξιολόγησης αν έχει ρυθμιστεί
...
1. **Αναπτυγμένη Υποδομή:**
      ...
      - Microsoft Foundry project με αναπτύξεις μοντέλων
...
2. **Αρχικός Πράκτορας:**
      ...
      - Ιχνηλάτηση και παρακολούθηση ρυθμισμένα
...
4. **Ετοιμότητα για Παραγωγή:**
      ...
      - Σάρωση ασφαλείας με red teaming
      - Παρακολούθηση απόδοσης
      - Πλαίσιο αξιολόγησης ποιότητας
      - Κλιμακωτή αρχιτεκτονική
...
Αποποίηση ευθύνης:
Το παρόν έγγραφο έχει μεταφραστεί με χρήση υπηρεσίας μετάφρασης τεχνητής νοημοσύνης [Co-op Translator](https://github.com/Azure/co-op-translator). Παρότι επιδιώκουμε την ακρίβεια, παρακαλούμε να σημειώσετε ότι οι αυτοματοποιημένες μεταφράσεις ενδέχεται να περιέχουν λάθη ή ανακρίβειες. Το πρωτότυπο έγγραφο στη γλώσσα του πρέπει να θεωρείται η αυθεντική πηγή. Για κρίσιμες πληροφορίες συνιστάται επαγγελματική μετάφραση από άνθρωπο. Δεν φέρουμε ευθύνη για τυχόν παρανοήσεις ή λανθασμένες ερμηνείες που προκύπτουν από τη χρήση αυτής της μετάφρασης.
