...
function Test-Prerequisites {
    ...
    # Check which PowerShell version dey
...
function Test-Authentication {
    ...
    try {
        ...
        # Make sure subscription access dey
...
function Test-Permissions {
    ...
    try {
        ...
        # Try create resource group (if dem specify am)
        if ($ResourceGroup) {
            ...
            if ($rgExists -eq "true") {
                ...
                # Test if e fit create resource group
                try {
...
function Test-TemplateValidation {
    ...
    # Check if infrastructure files dey
    if (Test-Path "infra") {
        $bicepFiles = Get-ChildItem -Path "infra" -Filter "*.bicep" -Recurse
        if ($bicepFiles.Count -gt 0) {
            Write-Status "Infrastructure templates" "Success" "$($bicepFiles.Count) Bicep files found"
            
            # Validate main.bicep if e dey
...
check_regional_availability() {
    ...
    # Check if service dey available
...
validate_prod_environment() {
    ...
    # Make sure say monitoring setup dey correct
...
def check_network_limits(location: str) -> bool:
    ...
    # Check di virtual networks
    vnets = run_command(['az', 'network', 'vnet', 'list'])
    if vnets is not None:
        vnet_count = len(vnets)
        print(f"✅ Virtual Networks: {vnet_count}/1000")
    
    # Check di public IP addresses
...
check_security_practices() {
    ...
    # Check if dem dey use Key Vault
...
check_compliance_requirements() {
    ...
    # Check if audit logging dey enabled
