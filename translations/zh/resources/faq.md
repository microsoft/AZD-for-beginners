...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T17:47:03+00:00",
...
这份全面的FAQ解答了关于Azure Developer CLI (azd)和Azure部署的常见问题。快速找到问题的解决方案，了解最佳实践，并深入理解azd的概念和工作流程。
...
定期参考这份FAQ，你将能够：
- 独立解决Azure Developer CLI的常见问题，使用提供的解决方案
- 就部署策略和配置做出明智决策
...
**答**：Azure Developer CLI (azd) 是一个以开发者为中心的命令行工具，可以加速将应用程序从本地开发环境部署到Azure的过程。它通过模板提供最佳实践，并帮助完成整个部署生命周期。

### 问：azd与Azure CLI有什么不同？
**答**：
- **Azure CLI**：通用工具，用于管理Azure资源
- **azd**：以开发者为中心的工具，用于应用程序部署工作流
- azd内部使用Azure CLI，但为常见开发场景提供更高层次的抽象
- azd包括模板、环境管理和部署自动化功能

### 问：使用azd需要安装Azure CLI吗？
**答**：是的，azd需要Azure CLI进行认证和部分操作。请先安装Azure CLI，然后再安装azd。
...
### 问：如何使用azd进行Azure认证？
**答**：使用`azd auth login`，它会打开一个浏览器窗口进行Azure认证。对于CI/CD场景，可以使用服务主体或托管身份。
...
### 问：azd可以用于CI/CD管道吗？
**答**：当然可以！azd专为CI/CD集成设计。使用服务主体进行认证，并设置环境变量进行配置。

### 问：如何在GitHub Actions中处理认证？
...
3. 使用`azd up`进行全面测试
4. 在GitHub上发布并添加适当的标签

### 问：可以不使用模板直接使用azd吗？
**答**：可以，使用`azd init`在现有项目中创建必要的配置文件。你需要手动配置`azure.yaml`和基础设施文件。

### 问：官方模板和社区模板有什么区别？
**答**：
- **官方模板**：由微软维护，定期更新，文档全面
- **社区模板**：由开发者创建，可能有特定的使用场景，质量和维护情况各异
...
**答**：可以，使用`azd provision`只部署模板中定义的基础设施组件。

### 问：如何部署到现有的Azure资源？
**答**：这比较复杂，无法直接支持。你可以：
1. 将现有资源导入到你的Bicep模板中
2. 在模板中引用现有资源
3. 修改模板以条件性创建或引用资源

### 问：可以使用Terraform代替Bicep吗？
**答**：目前azd主要支持Bicep/ARM模板。Terraform尚未正式支持，但可能存在社区解决方案。

---

## 配置与环境

### 问：如何管理不同的环境（开发、测试、生产）？
...
### 问：为什么`azd up`失败了？
**答**：常见原因：
1. **认证问题**：运行`azd auth login`
2. **权限不足**：检查你的Azure角色分配
...
### 问：如何在部署前估算成本？
**答**：
1. 查看模板的`main.bicep`以了解创建的资源
2. 使用Azure定价计算器选择具体的SKU
3. 先部署到开发环境以监控实际成本
...
### 问：可以将azd与Azure DevOps一起使用吗？
**答**：可以，azd适用于任何CI/CD系统。创建使用azd命令的Azure DevOps管道。

### 问：如何为azd或社区模板做贡献？
**答**：
1. **azd工具**：为[Azure/azure-dev](https://github.com/Azure/azure-dev)做贡献
2. **模板**：按照[模板指南](https://github.com/Azure-Samples/awesome-azd)创建模板
3. **文档**：为[MicrosoftDocs/azure-dev-docs](https://github.com/MicrosoftDocs/azure-dev-docs)做贡献

### 问：azd的路线图是什么？
**答**：查看[官方路线图](https://github.com/Azure/azure-dev/projects)了解计划的功能和改进。
