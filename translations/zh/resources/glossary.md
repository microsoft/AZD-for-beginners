...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T18:02:30+00:00",
  "source_file": "resources/glossary.md",
  "language_code": "zh"
}
-->
# 术语表 - Azure Developer CLI 术语

## 简介

本术语表全面定义了 Azure Developer CLI 和 Azure 云开发中使用的术语、概念和缩写。它是理解技术文档、解决问题以及高效沟通 azd 项目和 Azure 服务的重要参考。
...
**应用洞察**  
Azure 的应用性能监控 (APM) 服务，提供关于应用性能、可用性和使用情况的深入洞察。
...
**Bicep**  
由微软开发的领域特定语言 (DSL)，用于部署 Azure 资源。相比 ARM 模板提供更简单的语法，同时编译为 ARM。
...
**容器应用**  
Azure 的无服务器容器服务，允许运行容器化应用而无需管理复杂的基础设施。
...
**环境**  
表示应用特定实例的部署目标（例如开发、预生产、生产）。

**环境变量**  
以键值对形式存储的配置值，应用可在运行时访问。

**端点**  
应用或服务可以访问的 URL 或网络地址。
...
**主机**  
应用将部署到的 Azure 服务类型（例如 appservice、containerapp、function）。
...
**Node.js**  
基于 Chrome V8 JavaScript 引擎构建的 JavaScript 运行时，用于构建服务器端应用。
...
**资源组**  
Azure 资源的逻辑容器，共享相同的生命周期、权限和策略。

**资源令牌**  
azd 生成的唯一字符串，用于确保资源名称在部署中唯一。

**REST API**  
使用 HTTP 方法设计网络应用的架构风格。

**回滚**  
恢复到应用或基础设施配置的先前版本的过程。
...
**存储账户**  
Azure 服务，提供包括 Blob、文件、队列和表在内的数据对象的云存储。
...
### 开发术语
- **热重载**：在开发过程中自动更新应用而无需重启
- **构建管道**：用于构建和测试代码的自动化过程
- **部署槽**：应用服务中的预生产环境
