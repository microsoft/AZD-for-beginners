# 在 AZD 中部署 Microsoft SQL 数据库和 Web 应用
...
- <strong>数据库</strong>：带示例数据的 Azure SQL Database
- <strong>基础设施</strong>：使用 Bicep（模块化、可重用模板）进行配置
- <strong>部署</strong>：使用 `azd` 命令完全自动化
...
graph TD
    ...
    WebApp -- 安全连接<br/>已加密 --> SQL[Azure SQL 数据库<br/>产品表<br/>示例数据]
```

**资源部署：**
- <strong>资源组</strong>：存放所有资源的容器
- **App Service 计划**：基于 Linux 的托管（为节省成本使用 B1 级别）
- **Web 应用**：使用 Python 3.11 运行时的 Flask 应用
- **SQL Server**：托管数据库服务器，至少使用 TLS 1.2
- **SQL 数据库**：Basic 级别（2GB，适合开发/测试）
- **Application Insights**：监控和日志记录
- **Log Analytics 工作区**：集中日志存储

<strong>类比</strong>：可以把它想象成一家餐厅（Web 应用）和一个冷库（数据库）。顾客从菜单（API 端点）下单，厨房（Flask 应用）从冷库取出食材（数据）。餐厅经理（Application Insights）跟踪发生的一切。
...
**每个文件的作用：**
- **azure.yaml**：告诉 AZD 要部署什么以及在哪里部署
- **infra/main.bicep**：编排所有 Azure 资源
...
<strong>发生了什么</strong>：AZD 为你的部署创建本地配置。
...
<strong>发生了什么</strong>：AZD 部署所有基础设施（需 5-8 分钟）：
1. 创建资源组
...
这将在浏览器中打开已部署的 Web 应用，地址为 `https://app-<unique-id>.azurewebsites.net`
...
机密通过 Azure App Service 配置进行安全管理——<strong>绝不在源代码中硬编码</strong>。

**由 AZD 自动配置**：
- `SQL_CONNECTION_STRING`：带加密凭据的数据库连接字符串
- `APPLICATIONINSIGHTS_CONNECTION_STRING`：监控遥测端点
- `SCM_DO_BUILD_DURING_DEPLOYMENT`：启用自动依赖安装

<strong>机密的存储位置</strong>：
1. 在运行 `azd provision` 时，你通过安全提示提供 SQL 凭据
2. AZD 将这些存储在本地的 `.azure/<env-name>/.env` 文件中（已被 git 忽略）
3. AZD 将它们注入到 Azure App Service 配置中（静态加密）
4. 应用在运行时通过 `os.getenv()` 读取它们

### 本地开发

用于本地测试，请根据示例创建 `.env` 文件：
...
- <strong>版本控制</strong>：基础设施更改由 Git 跟踪
...
### 1. <strong>源代码中无密钥</strong>
- ✅ 凭据存储在 Azure App Service 配置中（已加密）
- ✅ `.env` 文件通过 `.gitignore` 排除在 Git 之外
- ✅ 通过预配期间的安全参数传递机密

### 2. <strong>加密连接</strong>
- ✅ SQL Server 使用最低 TLS 1.2
- ✅ Web 应用强制使用 HTTPS
- ✅ 数据库连接使用加密通道

### 3. <strong>网络安全</strong>
- ✅ SQL Server 防火墙配置为仅允许 Azure 服务
- ✅ 公共网络访问受限（可以通过私有终结点进一步锁定）
- ✅ Web 应用禁用 FTPS

### 4. <strong>认证与授权</strong>
- ⚠️ <strong>当前</strong>：SQL 认证（用户名/密码）
- ✅ <strong>生产环境建议</strong>：使用 Azure 托管标识实现无密码认证

<strong>升级到托管标识</strong>（用于生产环境）：
1. 在 Web 应用上启用托管身份
2. 授予该身份 SQL 权限
3. 更新连接字符串以使用托管标识
4. 移除基于密码的认证
...
- ✅ 已启用 SQL Database 审计（可配置以满足合规需求）
- ✅ 为所有资源添加了标签以便治理
...
- [ ] 启用 Web 应用防火墙（WAF）
- [ ] 实施 Azure Key Vault 以进行密钥轮换
- [ ] 配置 Microsoft Entra ID 身份验证
- [ ] 为所有资源启用诊断日志记录

## 成本优化

<strong>估算月度费用</strong>（截至 2025 年 11 月）：
...
1. <strong>用于学习时使用免费层</strong>：
   - App Service：F1 级别（免费，小时受限）
...
4. **开发与生产的 SKU 区别**：
   - <strong>开发</strong>：示例中使用的 Basic 级别
   - <strong>生产</strong>：使用带冗余的 Standard/Premium 级别

<strong>成本监控</strong>：
- 在 [Azure Cost Management](https://portal.azure.com/#view/Microsoft_Azure_CostManagement) 中查看费用
- 设置成本警报以避免意外
...
```
<strong>注意</strong>：免费层有使用限制（每天 60 分钟 CPU，无法始终运行）。
...
3. 单击 Application Insights 资源（`appi-<unique-id>`）
...
1. Azure Portal → SQL Database → 审计
...
<strong>设置警报</strong>：
为关键事件创建警报：
- 5 分钟内 HTTP 500 错误数 > 5
- 数据库连接失败
- 响应时间过高（>2 秒）

<strong>示例警报创建</strong>：
```sh
...
- 确保 SQL Server 完全配置好（可能需要 2-3 分钟）
...
<strong>解决方案</strong>:
- 检查测试完成后是否忘记运行 `azd down`
- 验证 SQL 数据库是否使用 Basic 层（而不是 Premium）
- 在 Azure 成本管理中查看费用
- 设置费用提醒
...
1. 列出资源组中的所有资源：
   ...
   ```
   <strong>预期</strong>: 6-7 个资源（Web App、SQL Server、SQL Database、App Service Plan、Application Insights、Log Analytics）
...
2. 测试所有 API 端点：
   ...
   curl https://app-<your-id>.azurewebsites.net/products/1
   ```
   <strong>预期</strong>: 全部返回有效的 JSON 且没有错误
...
<strong>成功标准</strong>：全部 6-7 个资源存在，所有端点返回数据，Live Metrics 显示活动。

---

### 练习 2：添加新的 API 端点（中级）

<strong>目标</strong>：为 Flask 应用扩展新的端点。

<strong>入门代码</strong>：当前端点位于 `src/web/app.py`

<strong>步骤</strong>:
1. 编辑 `src/web/app.py` 并在 `get_product()` 函数之后添加一个新端点：
   ```python
...
3. 测试新端点：
   ...
   ```
   <strong>预期</strong>: 返回与 "laptop" 匹配的产品

<strong>成功标准</strong>：新端点工作，返回筛选结果，并在 Application Insights 日志中出现。

---

### 练习 3：添加监控和警报（高级）

<strong>目标</strong>：设置主动监控和警报。
...
<strong>成功标准</strong>：警报规则已创建，在错误发生时触发，并接收到通知。

---

### 练习 4：数据库模式更改（高级）

<strong>目标</strong>：添加新表并修改应用以使用它。
...
<strong>成功标准</strong>：新表存在，产品显示类别信息，应用仍能工作。
...
<strong>提示</strong>：从 [Azure Cache for Redis documentation](https://learn.microsoft.com/azure/azure-cache-for-redis/) 开始阅读。
...
### 高级主题
- **托管标识（Managed Identity）**：移除密码并使用 Microsoft Entra ID 进行身份验证
- <strong>私有终结点</strong>：在虚拟网络内保护数据库连接
- **CI/CD 集成**：使用 GitHub Actions 或 Azure DevOps 自动化部署
- <strong>多环境</strong>：设置开发、暂存和生产环境
- <strong>数据库迁移</strong>：对模式版本控制使用 Alembic 或 Entity Framework
...
**把 AZD 想成**：适用于 Azure 的 Docker Compose——为复杂部署简化配置。
...
**问：我可以在生产环境中使用此项目吗？**  
答：这是一个起点。用于生产时，请添加：托管标识、私有终结点、冗余、备份策略、WAF 和增强的监控。

**问：如果我想使用容器而不是代码部署怎么办？**  
答：查看 [容器应用示例](../../../../examples/container-app)，其中在各处使用 Docker 容器。
...
**问：我可以使用现有数据库而不是新建一个吗？**  
答：可以，修改 `infra/main.bicep` 以引用现有的 SQL Server 并更新连接字符串参数。

---

> **注意：** 该示例演示了使用 AZD 部署带数据库的 Web 应用的最佳实践。它包含可运行的代码、全面的文档和用于巩固学习的实践练习。对于生产部署，请审查贵组织特定的安全性、伸缩性、合规性和成本要求。
