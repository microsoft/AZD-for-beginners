...
**學習目標：** 了解為零售商建立可生產使用的多代理客戶支援聊天機器人的架構、設計決策與實作方法，具備進階 AI 能力，包括庫存管理、文件處理與智慧客戶互動。

**完成時間：** 閱讀與理解（2-3 小時）｜建置完整實作（80-120 小時）
...
**教育重點：** 此架構示範企業級多代理系統的設計模式。

### 系統需求（供您實作）

一個生產環境的客戶支援解決方案需要：
- <strong>多個專精代理</strong> 以應對不同的客戶需求（客服 + 庫存管理）
- <strong>多模型部署</strong> 並妥善規劃容量（gpt-4.1、gpt-4.1-mini、跨區域的嵌入模型）
- <strong>動態資料整合</strong> 與 AI Search 及檔案上傳（向量搜尋 + 文件處理）
- <strong>完整的監控</strong> 與評估能力（Application Insights + 自訂指標）
- <strong>生產等級安全性</strong> 與紅隊驗證（漏洞掃描 + 代理評估）

### 本指南提供

✅ <strong>架構模式</strong> - 可擴充的多代理系統驗證設計  
✅ <strong>基礎設施範本</strong> - 部署所有 Azure 服務的 ARM 範本  
✅ <strong>程式碼範例</strong> - 關鍵元件的參考實作  
✅ <strong>設定指引</strong> - 逐步建立說明  
✅ <strong>最佳實務</strong> - 安全、監控、成本優化策略  

❌ <strong>不包含</strong> - 完整可運行的應用程式（需開發工作）

## 🗺️ 實作路線圖

### 階段 1：研究架構（2-3 小時） - 從此開始
...
- ✅ AI Search 服務（空的，需要索引設定）
- ✅ Container Apps 環境（預留映像）
...
- 為所有服務建置 Docker 映像
- 推到 Azure Container Registry
- 更新 Container Apps 使用實際映像
- 設定環境變數與金鑰
- 執行評估測試套件
- 進行安全掃描

**總估計工作量：** 經驗豐富的開發者需 80-120 小時
...
graph TB
    ...
    Router --> AppInsights[Application Insights<br/>監控]
...
| 元件 | 目的 | 技術 | 區域 |
|-----------|---------|------------|---------|
| **Web 前端** | 用戶與顧客互動的介面 | Container Apps | 主要區域 |
| <strong>代理路由器</strong> | 將請求路由至相應代理 | Container Apps | 主要區域 |
| <strong>客服代理</strong> | 處理客戶服務查詢 | Container Apps + gpt-4.1 | 主要區域 |
| <strong>庫存代理</strong> | 管理庫存與履行 | Container Apps + gpt-4.1-mini | 主要區域 |
| **Microsoft Foundry Models** | 為代理提供 LLM 推論 | Azure AI Services | 多區域 |
| **AI Search** | 向量搜尋與 RAG | AI Search Service | 主要區域 |
| <strong>儲存帳戶</strong> | 檔案上傳與文件 | Blob Storage | 主要區域 |
| **Application Insights** | 監控與遙測 | Monitor | 主要區域 |
| <strong>評分模型</strong> | 代理評估系統 | Microsoft Foundry Models | 次要區域 |
...
以下章節提供詳細的架構模式、設定範例與參考程式碼，以協助您實作。

## 初始設定需求

### 1. 多代理與設定

<strong>目標</strong>：部署 2 個專精代理 —「客服代理」（客戶服務）與「庫存」（庫存管理）

> **📝 注意：** 下列 azure.yaml 與 Bicep 設定為 <strong>參考範例</strong>，示範如何結構化多代理部署。您需要建立這些檔案與對應的代理實作。
...
<strong>目標</strong>：部署聊天模型（客服）、嵌入模型（搜尋）與推理模型（評分），並妥善管理配額
...
<strong>目標</strong>：設定 AI Search 以進行資料更新與自動索引
...
### 4. 代理工具的 AI Search 設定

<strong>目標</strong>：設定代理以使用 AI Search 作為資料依據工具
...
<strong>目標</strong>：使代理能處理上傳檔案（手冊、文件）以用於 RAG 上下文
...
<strong>目標</strong>：新增 Bing 搜尋功能以取得即時資訊
...
<strong>目標</strong>：使用追蹤日誌及 Application Insights 進行完整監控
...
<strong>目標</strong>：部署具專屬評分模型的評估系統
...
<strong>問題</strong>：因 API 版本過期導致模型部署失敗
...
def check_model_versions():
    ...
    # 這會呼叫 Microsoft Foundry Models API 以取得目前的版本
    latest_versions = {
        "gpt-4.1": "2024-11-20",
...
<strong>問題</strong>：如何將微調模型整合到 AZD 部署
...
**成本影響：**
- <strong>節省</strong>：對於簡單查詢可降低 60-80% 成本
- <strong>取捨</strong>：路由邏輯會帶來輕微延遲增加
- <strong>監控</strong>：追蹤準確度與成本指標

#### 問：我可以從 azd 範本啟動一個微調工作嗎？
...
> **✨ 這是真實存在且可運作的！**  
> 與上面的概念性程式碼範例不同，這個 ARM 範本是本儲存庫中包含的「真實、可運作的基礎設施部署」。

### 這個範本實際上會做什麼

位於 [`retail-multiagent-arm-template/`](../../../examples/retail-multiagent-arm-template) 的 ARM 範本會佈建多代理系統所需的<strong>所有 Azure 基礎設施</strong>。這是唯一一個能立即運行的組件 —— 其他部分都需要開發。
...
- ✅ **Multi-region Microsoft Foundry Models** 部署 (gpt-4.1, gpt-4.1-mini, embeddings, grader)
- ✅ **Azure AI Search**，具向量搜尋能力
- ✅ **Azure Storage**，含文件與上傳容器
- ✅ **Container Apps Environment**，具有自動縮放
- ✅ **Agent Router & Frontend** container apps
- ✅ **Cosmos DB**，用於聊天歷史的持久化
- ✅ **Application Insights**，提供完整監控
- ✅ **Key Vault**，安全的機密管理
- ✅ **Document Intelligence**，用於檔案處理
- ✅ **Bing Search API**，用於即時資訊
...
本情景文件與 ARM 範本結合，提供部署生產就緒多代理客服解決方案所需的一切。實作涵蓋：

✅ <strong>架構設計</strong> - 含元件關係的完整系統設計  
✅ <strong>基礎設施佈建</strong> - 可一鍵部署的完整 ARM 範本  
✅ **Agent 設定** - 客服與庫存代理的詳細設定  
✅ <strong>多模型部署</strong> - 不同區域的模型佈署策略  
✅ <strong>搜尋整合</strong> - AI Search 向量功能與資料索引  
✅ <strong>安全實作</strong> - 紅隊測試、弱點掃描與安全實務  
✅ <strong>監控與評估</strong> - 完整遙測與代理評估框架  
✅ <strong>生產準備</strong> - 企業級部署含 HA 與災難復原  
✅ <strong>成本優化</strong> - 智能路由與基於使用量的縮放  
...
✅ <strong>多代理系統設計</strong> - 專精代理（客服 + 庫存）與專屬模型  
✅ <strong>多區域部署</strong> - 策略性模型放置以節省成本並提升冗餘  
✅ **RAG 架構** - AI Search 與向量嵌入整合以獲得具根據的回應  
✅ <strong>代理評估</strong> - 使用專屬 grader 模型進行品質評估  
...
1. **基礎設施 vs 應用程式** - ARM 範本提供基礎設施；代理需進行開發
2. <strong>多區域策略</strong> - 策略性模型放置可降低成本並提升可靠性
3. <strong>評估框架</strong> - 專屬 grader 模型可持續進行品質評估
4. <strong>安全優先</strong> - 紅隊測試與弱點掃描對於生產環境至關重要
5. <strong>成本優化</strong> - 在 gpt-4.1 與 gpt-4.1-mini 之間進行智慧路由可節省 60-80%
...
- 📖 [常見問題](../resources/faq.md) - 常見問題的解答
- 🔧 [疑難排解指南](../docs/troubleshooting/common-issues.md) - 部署問題

---

**此完整情景提供企業級多代理 AI 系統的架構藍圖，包含基礎設施範本、實作指南與生產最佳實務，協助使用 Azure Developer CLI 建置進階的客服解決方案。**
