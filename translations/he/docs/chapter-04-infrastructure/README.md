# פרק 4: תשתית כקוד ופריסה

**📚 קורס**: [AZD למתחילים](../../README.md) | **⏱️ משך**: 1-1.5 שעות | **⭐ מורכבות**: בינוני

---

## מבוא

פרק זה עוסק בתבניות תשתית כקוד (IaC) עם תבניות Bicep, אספקת משאבים, ואסטרטגיות פריסה באמצעות Azure Developer CLI.

> אומת נגד `azd 1.27.1` ביולי 2026.

## מטרות הלמידה

בסיום פרק זה תוכל:
- להבין את מבנה וסינטקס תבניות Bicep
- לספק משאבים ב-Azure עם `azd provision`
- לפרוס יישומים עם `azd deploy`
- ליישם אסטרטגיות פריסה של כחול-ירוק ופריסה מתגלגלת

---

## 📚 שיעורים

| # | שיעור | תיאור | זמן |
|---|--------|-------------|------|
| 1 | [אספקת משאבים](provisioning.md) | ניהול משאבי Azure עם AZD | 45 דקות |
| 2 | [מדריך פריסה](deployment-guide.md) | אסטרטגיות פריסת יישומים | 45 דקות |
| 3 | [כתיבת התבנית שלך](custom-templates.md) | בנייה ופרסום תבניות azd לשימוש חוזר | 30 דקות |

---

## 🚀 התחלה מהירה

```bash
# אתחול מתבנית
azd init --template azure-functions-python-v2-http

# תצוגה מקדימה של מה שיווצר
azd provision --preview

# הקמת תשתית בלבד
azd provision

# פריסת קוד בלבד
azd deploy

# או שניהם יחד
azd up
```

---

## 📁 מבנה פרויקט AZD

```
my-project/
├── azure.yaml            # ← Project definition
├── infra/                # ← Infrastructure as Code
│   ├── main.bicep        # Entry point
│   ├── main.parameters.json
│   └── modules/          # Reusable components
│       ├── web.bicep
│       └── database.bicep
└── src/                  # Application code
    └── api/
```

---

## 🔧 פקודות חיוניות

| פקודה | תיאור |
|---------|-------------|
| `azd init` | אתחול הפרויקט |
| `azd provision` | יצירת משאבי Azure |
| `azd deploy` | פריסת קוד היישום |
| `azd up` | אספקה + פריסה |
| `azd down` | מחיקת כל המשאבים |

---

## 🔗 ניווט

| כיוון | פרק |
|-----------|---------|
| **קודם** | [פרק 3: קונפיגורציה](../chapter-03-configuration/README.md) |
| **הבא** | [פרק 5: פתרונות רב-סוכנים](../chapter-05-multi-agent/README.md) |

---

## 📖 משאבים קשורים

- [בדיקות לפני פריסה](../chapter-06-pre-deployment/README.md)
- [דוגמאות לאפליקציית מכולות](../../examples/container-app/README.md)
- [דוגמא לאפליקציית מסד נתונים](../../examples/database-app/README.md)

---

<!-- CO-OP TRANSLATOR DISCLAIMER START -->
**כתב ויתור**:
מסמך זה תורגם באמצעות שירות תרגום אוטומטי [Co-op Translator](https://github.com/Azure/co-op-translator). למרות שאנו שואפים לדיוק, יש לקחת בחשבון שתרגומים אוטומטיים עלולים להכיל שגיאות או אי-דיוקים. יש להחשיב את המסמך המקורי בשפתו הטבעית כמקור הסמכות. למידע קריטי מומלץ להשתמש בתרגום מקצועי על ידי מתרגם אדם. אנו לא אחראים לכל אי-הבנה או פירוש שגוי הנובע מהשימוש בתרגום זה.
<!-- CO-OP TRANSLATOR DISCLAIMER END -->