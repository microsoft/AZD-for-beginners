...
Hướng dẫn này cung cấp các thực hành tốt nhất toàn diện để triển khai các workload AI sẵn sàng cho môi trường sản xuất bằng Azure Developer CLI (AZD). Dựa trên phản hồi từ cộng đồng Microsoft Foundry Discord và các triển khai khách hàng thực tế, các thực hành này giải quyết những thách thức phổ biến nhất trong hệ thống AI sản xuất.

## Key Challenges Addressed

Dựa trên kết quả khảo sát cộng đồng của chúng tôi, đây là những thách thức hàng đầu mà các nhà phát triển phải đối mặt:

- **45%** gặp khó khăn với triển khai AI đa dịch vụ
- **38%** gặp vấn đề với quản lý chứng thực và bí mật  
- **35%** thấy việc sẵn sàng cho sản xuất và mở rộng quy mô khó khăn
- **32%** cần các chiến lược tối ưu hóa chi phí tốt hơn
- **29%** yêu cầu cải thiện giám sát và gỡ lỗi
...
- Không cho phép giao tiếp giữa các dịch vụ nếu không có xác thực
...
class TokenOptimizer {
  ...
  optimizePrompt(userInput: string, context: string): string {
    ...
    if (estimatedTokens > availableTokens) {
      // Cắt bớt ngữ cảnh, không cắt bớt đầu vào của người dùng
...
# Xác minh các triển khai mô hình OpenAI
...
- [ ] RBAC với nguyên tắc quyền tối thiểu
...
- [ ] Đã tạo bảng điều khiển
- [ ] Đã triển khai kiểm tra sức khỏe
...
|--------|--------|------------|
| **Response Time** | < 2 giây | Application Insights |
| **Availability** | 99.9% | Giám sát thời gian hoạt động |
| **Error Rate** | < 0.1% | Log ứng dụng |
| **Token Usage** | < $500/tháng | Quản lý chi phí |
| **Concurrent Users** | 1000+ | Kiểm thử tải |
| **Recovery Time** | < 1 giờ | Kiểm tra phục hồi thảm họa |
...
1. **Start Small, Scale Gradually**: Bắt đầu với các SKUs cơ bản và mở rộng dựa trên mức sử dụng thực tế
2. **Monitor Everything**: Thiết lập giám sát toàn diện ngay từ ngày đầu
3. **Automate Security**: Sử dụng hạ tầng như mã để đảm bảo bảo mật nhất quán
4. **Test Thoroughly**: Bao gồm kiểm thử chuyên biệt cho AI trong pipeline của bạn
5. **Plan for Costs**: Giám sát việc sử dụng token và thiết lập cảnh báo ngân sách sớm

### Common Pitfalls to Avoid:

- ❌ Gắn cứng API key trong mã
...
**Remember**: Các workload AI trong môi trường sản xuất đòi hỏi lập kế hoạch cẩn thận, giám sát và tối ưu hóa liên tục. Bắt đầu với các mẫu này và điều chỉnh chúng phù hợp với yêu cầu cụ thể của bạn.

---

<!-- CO-OP TRANSLATOR DISCLAIMER START -->
Miễn trừ trách nhiệm:
Tài liệu này đã được dịch bằng dịch vụ dịch thuật tự động AI Co-op Translator (https://github.com/Azure/co-op-translator). Mặc dù chúng tôi cố gắng đảm bảo độ chính xác, xin lưu ý rằng các bản dịch tự động có thể chứa lỗi hoặc sai sót. Tài liệu gốc bằng ngôn ngữ nguyên bản phải được coi là nguồn có thẩm quyền. Đối với những thông tin quan trọng, nên sử dụng bản dịch chuyên nghiệp do người dịch thực hiện. Chúng tôi không chịu trách nhiệm về bất kỳ hiểu lầm hoặc diễn giải sai nào phát sinh từ việc sử dụng bản dịch này.
