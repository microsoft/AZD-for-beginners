...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-09T22:43:21+00:00",
...
Ang komprehensibong FAQ na ito ay nagbibigay ng mga sagot sa mga karaniwang tanong tungkol sa Azure Developer CLI (azd) at mga deployment sa Azure. Makahanap ng mabilis na solusyon sa mga karaniwang problema, maunawaan ang mga pinakamahusay na kasanayan, at magkaroon ng kaliwanagan sa mga konsepto at workflow ng azd.
...
- Maiintindihan ang mga pangunahing konsepto at terminolohiya sa pamamagitan ng praktikal na format ng Q&A
- Makakakuha ng mga solusyon sa troubleshooting para sa mga madalas na problema at error
...
- Makakapag-resolba ng mga karaniwang isyu sa Azure Developer CLI nang mag-isa gamit ang mga ibinigay na solusyon
- Makakagawa ng mas maalam na desisyon tungkol sa mga deployment strategy at configuration
- Maiintindihan ang relasyon sa pagitan ng azd at iba pang mga tool at serbisyo ng Azure
- Maipapatupad ang mga pinakamahusay na kasanayan batay sa karanasan ng komunidad at rekomendasyon ng mga eksperto
...
- [Configuration at Mga Environment](../../../resources)
...
**S**: Ang Azure Developer CLI (azd) ay isang developer-centric na command-line tool na nagpapabilis sa oras para ma-deploy ang iyong application mula sa lokal na development environment papunta sa Azure. Nagbibigay ito ng mga pinakamahusay na kasanayan sa pamamagitan ng mga template at tumutulong sa buong lifecycle ng deployment.

### T: Paano naiiba ang azd sa Azure CLI?
**S**: 
- **Azure CLI**: Pangkaraniwang tool para sa pamamahala ng mga resource sa Azure
- **azd**: Tool na nakatuon sa mga workflow ng application deployment
- Ginagamit ng azd ang Azure CLI sa loob ngunit nagbibigay ng mas mataas na antas ng abstraction para sa mga karaniwang development scenario
- Kasama sa azd ang mga template, pamamahala ng environment, at automation ng deployment

### T: Kailangan ko bang naka-install ang Azure CLI para magamit ang azd?
**S**: Oo, kailangan ng azd ang Azure CLI para sa authentication at ilang operasyon. I-install muna ang Azure CLI bago i-install ang azd.
...
**S**: Oo! Maaari mong:
1. Gamitin ang `azd init` para magdagdag ng azd configuration sa mga umiiral na proyekto
2. I-adapt ang mga umiiral na proyekto upang tumugma sa istruktura ng azd template
3. Gumawa ng custom na template batay sa iyong umiiral na arkitektura
...
**S**: Gamitin ang `azd auth login` na magbubukas ng browser window para sa authentication sa Azure. Para sa mga CI/CD scenario, gumamit ng service principals o managed identities.

### T: Maaari ko bang gamitin ang azd sa maraming Azure subscription?
**S**: Oo. Gamitin ang `azd env set AZURE_SUBSCRIPTION_ID <subscription-id>` para tukuyin kung aling subscription ang gagamitin para sa bawat environment.
...
- Ang mga partikular na permiso ay nag-iiba batay sa template at mga resource na ide-deploy
...
- Mga template ng komunidad: Maghanap sa GitHub ng "azd-template"
- Gamitin ang `azd template list` para mag-browse ng mga available na template
...
**S**: Oo, gamitin ang `azd init` sa isang umiiral na proyekto para gumawa ng kinakailangang configuration files. Kailangan mong mano-manong i-configure ang `azure.yaml` at mga infrastructure file.
...
- **Community templates**: Ginawa ng mga developer, maaaring may specialized na use cases, iba-iba ang kalidad at maintenance

### T: Paano ko ia-update ang template sa aking proyekto?
**S**: Ang mga template ay hindi awtomatikong ina-update. Maaari mong:
...
3. Piliin ang mga partikular na improvement mula sa mga updated na template

---

## Deployment at Infrastructure

### T: Anong mga Azure service ang maaaring i-deploy ng azd?
**S**: Maaaring mag-deploy ang azd ng anumang Azure service sa pamamagitan ng Bicep/ARM templates, kabilang ang:
...
**S**: Oo, i-configure ang maraming rehiyon sa iyong Bicep templates at itakda ang location parameter nang naaayon para sa bawat environment.
...
### T: Maaari ba akong mag-deploy ng infrastructure lang nang walang application?
**S**: Oo, gamitin ang `azd provision` para mag-deploy ng mga infrastructure component lang na tinukoy sa iyong mga template.

### T: Paano ako mag-deploy sa mga umiiral na Azure resource?
**S**: Ito ay medyo kumplikado at hindi direktang sinusuportahan. Maaari mong:
1. I-import ang mga umiiral na resource sa iyong Bicep templates
2. Gumamit ng mga umiiral na resource reference sa mga template
3. I-modify ang mga template para kondisyonal na gumawa o mag-refer sa mga resource
...
## Configuration at Mga Environment

### T: Paano ko pamamahalaan ang iba't ibang environment (dev, staging, prod)?
**S**: Gumawa ng hiwalay na environment gamit ang `azd env new <environment-name>` at i-configure ang iba't ibang setting para sa bawat isa:
```bash
...
### T: Saan naka-store ang mga configuration ng environment?
**S**: Sa `.azure` folder sa loob ng iyong project directory. Ang bawat environment ay may sariling folder na may configuration files.

### T: Paano ako magse-set ng environment-specific configuration?
...
### T: Maaari ko bang i-share ang mga configuration ng environment sa mga miyembro ng team?
...
2. Gumamit ng deployment scripts para mag-set up ng mga environment
...
### T: Bakit nabibigo ang `azd up`?
**S**: Karaniwang mga sanhi:
...
### T: Paano ko ide-debug ang mga deployment failure?
**S**: 
1. Gamitin ang `azd deploy --debug` para sa verbose output
2. Suriin ang deployment history sa Azure portal
...
3. Ang environment ay napili: `azd env select <environment>`
4. Ang mga variable ay naka-set sa tamang environment
...
```
Tatanggalin nito ang lahat ng resource at configuration ng environment.
...
3. Ang mga network security group ay nagpapahintulot ng traffic
...
### T: Magkano ang magiging gastos ng azd deployments?
**S**: Ang mga gastos ay nakadepende sa:
- Mga Azure service na ide-deploy
- Mga service tier/SKU na napili
- Mga pagkakaiba sa regional pricing
...
**S**: 
1. Gumamit ng mas mababang-tier na SKU para sa development environments
2. Mag-set up ng Azure budgets at alerts
3. Gamitin ang `azd down` para tanggalin ang mga resource kapag hindi kailangan
4. Pumili ng naaangkop na rehiyon (nag-iiba ang gastos batay sa lokasyon)
5. Gumamit ng mga tool sa Azure Cost Management

### T: May mga free tier options ba para sa azd templates?
**S**: Maraming Azure service ang nag-aalok ng free tiers:
...
I-configure ang mga template para gamitin ang free tiers kung available.

### T: Paano ko tinatantya ang gastos bago ang deployment?
**S**: 
1. I-review ang `main.bicep` ng template para makita kung anong mga resource ang gagawin
2. Gumamit ng Azure Pricing Calculator na may partikular na SKU
...
2. I-reference ang mga Key Vault secrets sa application configuration
...
**S**: 
1. Gumamit ng hiwalay na environment para sa bawat stage (dev/staging/prod)
...
2. I-standardize ang azd templates sa mga team
...
### T: Paano ako magko-contribute sa azd o gagawa ng community templates?
**S**: 
1. **azd tool**: Mag-contribute sa [Azure/azure-dev](https://github.com/Azure/azure-dev)
2. **Templates**: Gumawa ng mga template na sumusunod sa [template guidelines](https://github.com/Azure-Samples/awesome-azd)
3. **Documentation**: Mag-contribute sa docs sa [MicrosoftDocs/azure-dev-docs](https://github.com/MicrosoftDocs/azure-dev-docs)

### T: Ano ang roadmap para sa azd?
**S**: Tingnan ang [opisyal na roadmap](https://github.com/Azure/azure-dev/projects) para sa mga planong tampok at improvement.

### T: Paano ako magmi-migrate mula sa ibang deployment tools papunta sa azd?
**S**:
...
5. Unti-unting ilipat ang mga environment  
...
### **Maghanap Muna**
- Suriin ang [opisyal na dokumentasyon](https://learn.microsoft.com/en-us/azure/developer/azure-developer-cli/)  
...
- [GitHub Issues](https://github.com/Azure/azure-dev/issues/new) - Para sa mga ulat ng bug at mga kahilingan sa bagong tampok  
- Isama ang mga kaugnay na log, mga mensahe ng error, at mga hakbang upang maulit ang problema  
