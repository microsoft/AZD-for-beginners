...
- **➡️ Следеће поглавље**: [Поглавље 5: Мулти-агентска AI решења](../../examples/retail-scenario.md)
- **🔧 Повезано**: [Поглавље 6: Валидација пре распоређивања](../chapter-06-pre-deployment/capacity-planning.md)

## Увод

Ова свеобухватна упутство покрива све што треба да знате о провизионисању и управљању Azure ресурсима користећи Azure Developer CLI. Научите како да имплементирате обрасце Инфраструктура као код (IaC) од основног креирања ресурса до напредних архитектура инфраструктуре за предузећа користећи Bicep, ARM шаблоне, Terraform и Pulumi.

## Циљеви учења

Након завршетка овог водича, ви ћете:
- Усвојити принципе Инфраструктура као код и провизионисање Azure ресурса
- Разумети више IaC провајдера које подржава Azure Developer CLI
...
- Имплементирати напредне инфраструктурне обрасце укључујући мрежу и безбедност
- Управљати животним циклусом ресурса, ажурирањима и разрешавањем зависности
...
Azure Developer CLI подржава више провајдера Инфраструктура као код (IaC):
- **Bicep** (препоручено) - Azure-ов језик специфичан за домен
- **ARM Templates** - JSON-базирани Azure Resource Manager шаблони
...
- **Рачунарски ресурси**: App Service, Container Apps, Functions, Virtual Machines
...
## 🗃️ Провизионисање базе података
...
Команда `azd provision --preview` вам омогућава да **симулирате провизионисање инфраструктуре** пре него што заправо распоредите ресурсе. То је слично духу `terraform plan` или `bicep what-if`, дајући вам **преглед без примене** шта би било промењено у вашем Azure окружењу.

#### 🛠️ Шта то ради
- **Анализира ваше IaC шаблоне** (Bicep или Terraform)
- **Приказује преглед измена ресурса**: додавања, брисања, ажурирања
- **Не примењује измене** — само за читање и безбедно за покретање
...
- **Валидирате измене инфраструктуре** пре него што примените ресурсе
- **Откријете погрешне конфигурације рано** у развојном циклусу
- **Сарадња на безбедан начин** у тимским окружењима
- **Омогућите распоређивања са најмање привилегија** без изненађења

Посебно је корисно када:
- Радите са комплексним окружењима са више услуга
- Правите измене у продукционој инфраструктури
- Валидамо измене шаблона пре одобрења PR-а
- Обучавате нове чланове тима о образцима инфраструктуре
...
```bash
# Прво прегледајте промене инфраструктуре (ПРЕПОРУЧЕНО)
azd provision --preview

# Примените промене након потврде прегледа
azd provision --confirm-with-no-prompt

# За повратак на претходно стање користите Git да поништите промене инфраструктуре:
...
- [Планирање пре распоређивања](../chapter-06-pre-deployment/capacity-planning.md) - Валидација доступности ресурса
- [Уобичајени проблеми](../chapter-07-troubleshooting/common-issues.md) - Решавање проблема инфраструктуре
- [Водич за дебаговање](../chapter-07-troubleshooting/debugging.md) - Дебаговање проблема при провизионисању
- [Избор SKU-а](../chapter-06-pre-deployment/sku-selection.md) - Избор одговарајућих нивоа услуга

## Додатни ресурси

- [Документација Azure Bicep](https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/)
