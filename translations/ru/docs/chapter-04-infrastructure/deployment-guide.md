# Руководство по развертыванию - Осваиваем развертывания AZD
...
- **➡️ Далее**: [Развертывание ресурсов](provisioning.md)
- **🚀 Следующая глава**: [Глава 5: Многоагентные AI решения](../../examples/retail-scenario.md)

## Введение

Это подробное руководство охватывает все, что вам нужно знать о развертывании приложений с использованием Azure Developer CLI — от базовых развертываний одной командой до сложных производственных сценариев с пользовательскими хуками, несколькими средами и интеграцией CI/CD. Освойте полный жизненный цикл развертывания с практическими примерами и лучшими практиками.

## Цели обучения

Пройдя это руководство, вы сможете:
- Овладеть всеми командами и рабочими процессами развертывания Azure Developer CLI
- Понять полный жизненный цикл развертывания — от создания ресурсов до мониторинга
- Реализовать пользовательские хуки развертывания для автоматизации до и после развертывания
- Настроить несколько сред с параметрами, специфичными для среды
- Настроить продвинутые стратегии развертывания, такие как blue-green и canary развертывания
- Интегрировать развертывания azd с CI/CD конвейерами и DevOps рабочими процессами

## Результаты обучения

После завершения вы сможете:
- Независимо выполнять и устранять неполадки всех рабочих процессов развертывания azd
- Проектировать и реализовывать пользовательскую автоматизацию развертывания с использованием хуков
- Настраивать производственные развертывания с правильной безопасностью и мониторингом
...
Когда необходимо только обновить ресурсы Azure:
```bash
# Обеспечение/обновление инфраструктуры
azd provision

# Обеспечение с режимом тестирования для предварительного просмотра изменений
...
После любого развертывания убедитесь в его успешности:

```bash
# Проверьте, что все сервисы работают
azd show

# Тестируйте конечные точки состояния
...
# Отслеживайте ошибки (открывается в браузере по умолчанию)
azd monitor --logs
```

**Критерии успеха:**
- ✅ Все сервисы имеют статус "Работает"
- ✅ Здоровые эндпоинты возвращают HTTP 200
- ✅ В журналах нет ошибок за последние 5 минут
...
### Настройки развертывания для сервисов
```yaml
# azure.yaml
services:
  web:
...
# Переключить трафик на синий (ручное обновление DNS/балансировщика нагрузки)
...
## 🐳 Развертывания с контейнерами

### Развертывание Container App
```yaml
services:
  api:
    project: ./src/api
...
### Развертывания с учётом сервисов
```bash
# Развернуть конкретный сервис для более быстрого итеративного процесса
azd deploy --service web
azd deploy --service api
...
# Это пропускает создание инфраструктуры и работает гораздо быстрее
azd deploy
...
### Управление личностью и доступом
```yaml
services:
  api:
    project: ./src/api
...
# Для отката инфраструктуры используйте контроль версий:
...
- [Развертывание ресурсов](provisioning.md) — глубокое погружение в управление инфраструктурой
...
**Цель**: Освоить различия между полным и инкрементальным развертыванием
...
- [ ] Изменения кода отражены в развернутом приложении
- [ ] Инфраструктура остаётся без изменений после `azd deploy`

**Результат обучения**: `azd deploy` на 50-70% быстрее `azd up` для изменений кода

### Упражнение 2: Пользовательские хуки развертывания (30 минут)
**Цель**: Реализовать автоматизацию до и после развертывания

```bash
# Создать скрипт валидации перед развертыванием
mkdir -p scripts
...
# Создать тест дымового тестирования после развертывания
...
**Критерии успеха:**
- [ ] Скрипт до развертывания запускается перед развертыванием
- [ ] Развертывание прерывается при провале тестов
- [ ] Пост-развертывательный smoke-тест проверяет здоровье
- [ ] Хуки выполняются в правильном порядке

### Упражнение 3: Стратегия многосредового развертывания (45 минут)
**Цель**: Реализовать пошаговый рабочий процесс развертывания (dev → staging → production)
...
# Шаг 3: Ручное одобрение для продакшена
echo "
...
**Критерии успеха:**
- [ ] Развертывание в dev среде прошло успешно
- [ ] Развертывание в staging среде прошло успешно
- [ ] Для production требуется ручное одобрение
- [ ] Во всех средах работают проверки здоровья
